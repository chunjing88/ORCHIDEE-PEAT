...
!!!qcj++ peatland
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: fwet_new
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: wtp
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: fpeat
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: wtpmax 
  REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE    :: shumdiag_peat
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: mc_peat_above
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: peat_PET
  REAL(r_std),DIMENSION(:),ALLOCATABLE      :: liqwt_ratio
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: veget_cov_max_new_peatdgvm 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE      :: wtp_year
  LOGICAL, DIMENSION(:),ALLOCATABLE            :: wettile_dgvm
  CHARACTER(LEN=25), DIMENSION(:),ALLOCATABLE  :: tile_name_dgvm

!  REAL(r_std),DIMENSION (:,:),ALLOCATABLE   :: veget_max_adjusted
!gmjc
...
!!!qcj++ peatland
  ...
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (peat_PET(kjpindex),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (liqwt_ratio(kjpindex),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (veget_cov_max_new_peatdgvm(kjpindex,nvm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (wtp_year(kjpindex,nvm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (wettile_dgvm(nstm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (tile_name_dgvm(nstm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
...
!-
  IF (printlev_loc>=3) WRITE(numout,*) "Call slowproc for restart."
       CALL slowproc_finalize (itau_fin,   kjpindex,    rest_id_sec, indices,    &
                               ...
                               assim_param_x,frac_age_x,altmax,fpeat,wtpmax,wtp_year)
!!$  CALL slowproc_main &
