...
!!!qcj++ peatland
  REAL(r_std),DIMENSION(:),ALLOCATABLE :: height_acro_g
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: carbon_acro_g
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: carbon_cato_g
  REAL(r_std),DIMENSION(:),ALLOCATABLE :: height_acro
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: carbon_acro
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: carbon_cato
  REAL(r_std),DIMENSION(:),ALLOCATABLE :: height_cato
  REAL(r_std),DIMENSION(:),ALLOCATABLE :: tcarbon_acro
  REAL(r_std),DIMENSION(:),ALLOCATABLE :: tcarbon_cato
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: resp_acro_oxic
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: resp_acro_anoxic
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: resp_cato
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: litter_to_acro
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: acro_to_cato
!  REAL(r_std),DIMENSION(:,:),ALLOCATABLE  :: wtp_pt_g
  REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE  :: wtp_pt
  REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE  :: deepC_peat
  REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE  :: deepC_peat_g
  REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE :: root_exudate
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: wtpmax_g 
  REAL(r_std),DIMENSION(:,:),ALLOCATABLE :: wtpmax
...
!!!qcj++ peatland
  ...
  IF (ier /= 0) CALL ipslerr_p(3, 'forcesoil', 'is_shrubpeat : error in memory allocation', '', '')
  DO i = 1, nvm
     is_shrubpeat(i) = is_shrubpeat_mtc(pft_to_mtc(i))
  ENDDO

  ALLOCATE(is_mosspeat(nvm),stat=ier)
  IF (ier /= 0) CALL ipslerr_p(3, 'forcesoil', 'is_mosspeat : error in memory allocation', '', '')
  DO i = 1, nvm
     is_mosspeat(i) = is_mosspeat_mtc(pft_to_mtc(i))
  ENDDO

  ALLOCATE(is_mineralwet(nvm),stat=ier)
  IF (ier /= 0) CALL ipslerr_p(3, 'forcesoil', 'is_mineralwet : error in memory allocation', '', '')
  DO i = 1, nvm
     is_mineralwet(i) = is_mineralwet_mtc(pft_to_mtc(i))
  ENDDO
...
!!!qcj++ peatland
         IF (ok_peat) THEN
           ALLOCATE(carbon_acro_g(nbp_glo,nvm),stat=ier)
           ALLOCATE(carbon_cato_g(nbp_glo,nvm),stat=ier)
           ALLOCATE(height_acro_g(nbp_glo),stat=ier)
           IF (ier /= 0) CALL ipslerr(3, 'forcesoil', 'peat carbon_g: error in memory allocation', '', '')
           carbon_acro_g(:,:) = val_exp
           var_name = 'carbon_acro'
           CALL restget (rest_id_sto, var_name, nbp_glo, nvm, 1, itau_dep, &
                &       .TRUE.,carbon_acro_g, "gather", nbp_glo, indices_g)
           IF ( ALL( carbon_acro_g(:,:) == val_exp ) ) carbon_acro_g(:,:)=zero

           carbon_cato_g(:,:) = val_exp
           var_name = 'carbon_cato'
           CALL restget (rest_id_sto, var_name, nbp_glo, nvm, 1, itau_dep, &
                &       .TRUE.,carbon_cato_g, "gather", nbp_glo, indices_g)
           IF (ALL(carbon_cato_g(:,:) == val_exp)) carbon_cato_g(:,:) = zero

           height_acro_g(:) = val_exp
           var_name = 'height_acro'
           CALL restget (rest_id_sto, var_name, nbp_glo, 1, 1,itau_dep, &
                &       .TRUE.,height_acro_g, "gather", nbp_glo, indices_g)
           IF (ALL(height_acro_g(:) == val_exp)) height_acro_g(:) = zero
           WRITE(numout,*) "date0 : ",date0, itau_dep
         ENDIF
...
!!!qcj++ peatland
           IF (perma_peat) THEN
               ALLOCATE(deepC_peat_g(nbp_glo,ndeep,nvm), stat=ier)
               IF (ier /= 0) CALL ipslerr(3, 'forcesoil', 'deepC_peat_g : error in memory allocation', '', '')

               deepC_peat_g(:,:,:) = val_exp
               CALL restget (rest_id_sto, 'deepC_peat', nbp_glo, ndeep, nvm, itau_dep, &
                   &               .TRUE., deepC_peat_g, 'gather', nbp_glo, indices_g)
               IF (ALL(deepC_peat_g == val_exp)) deepC_peat_g = zero

               ALLOCATE(wtpmax_g(nbp_glo,nvm), stat=ier)
               IF (ier /= 0) CALL ipslerr(3, 'forcesoil', 'wtpmax_g : error in memory allocation', '', '')

               var_name = 'wtpmax'
               wtpmax_g(:,:) = val_exp
               CALL restget (rest_id_sto, var_name, nbp_glo, nvm, 1,itau_dep, .TRUE.,wtpmax_g, "gather", nbp_glo, indices_g)
               IF (ALL(wtpmax_g(:,:) == val_exp)) THEN
                 CALL ipslerr(3, 'forcesoil', 'wtpmax is not found in stomate restart file', '', '')
               ENDIF
           ENDIF
           ...
           IF (reset_soilc) THEN
              CALL ipslerr(1, 'forcesoil', 'deepC_a, deepC_s and deeC_p, deepC_peat',  & 
...
!!!qcj++ peatland
     IF (ok_peat) THEN
        ...
        IF (ier /= 0) CALL ipslerr(3, 'forcesoil', 'wtp_pt : error in memory allocation', '', '')
        start_3d=(/ nbp_mpi_para_begin(mpi_rank), 1, 1 /)
        count_3d=(/  nbp_mpi_para(mpi_rank),nvm, nparan*nbyear /)
        CALL nccheck( NF90_INQ_VARID (Cforcing_id,'wtp_pt',v_id))
        CALL nccheck( NF90_GET_VAR (Cforcing_id,v_id,wtp_pt, &  
  ...
  ELSE
     ...
     !-
!!! qcj++ peatland
     ALLOCATE(root_exudate(kjpindex,nvm,nparan*nbyear), stat=ier)
     IF (ier /= 0) CALL ipslerr(3, 'forcesoil', 'root_exudate : error in memory allocation', '', '')
...
!!!qcj++ peatland
     IF (ok_peat) THEN 
        ...
        IF (ier /= 0) CALL ipslerr_p(3, 'forcesoil', 'peatland: error in memory allocation', '', '')
     ENDIF
  ELSE
...
!!!qcj++ peatland
     IF (perma_peat) THEN
        ...
        ALLOCATE(wtpmax(kjpindex,nvm), stat=ier)
        IF (ier /= 0) CALL ipslerr_p(3, 'forcesoil', 'wtpmax : error in memory allocation', '', '')
     ENDIF
...
!!!qcj peatland++
  IF (ok_peat) THEN
     CALL Scatter(carbon_acro_g,carbon_acro)
     CALL Scatter(carbon_cato_g,carbon_cato)
     CALL Scatter(height_acro_g,height_acro)
  ENDIF
...
!!!qcj++ peatland
     IF (perma_peat) THEN
         CALL Scatter(deepC_peat_g,deepC_peat)
         CALL Scatter(wtpmax_g,wtpmax)
     ENDIF
   ENDIF
!-
...
!!!qcj++ peatland
             deepC_peat,wtpmax,root_exudate(:,:,iatt))
      ENDDO
  ...
  DO i = 1,nvm
    IF (is_peat(i)) THEN
       WRITE(numout,*) 'QCJ check forcesoil, PFT',i,'is_peat'
    ELSE
       WRITE(numout,*) 'QCJ check forcesoil, PFT',i,'non peat'
    ENDIF
  ENDDO
  !!-
...
!!!qcj++ peatland
  IF (ok_peat) THEN
     CALL restput_p(rest_id_sto, 'carbon_acro', nbp_glo, nvm, 1, itau_dep, &
         &     carbon_acro, 'scatter',  nbp_glo, indices_g)
     CALL restput_p(rest_id_sto, 'carbon_cato', nbp_glo, nvm, 1, itau_dep, &
         &     carbon_cato, 'scatter',  nbp_glo, indices_g)
     CALL restput_p(rest_id_sto, 'height_acro', nbp_glo, 1, 1,itau_dep, &
         &     height_acro, 'scatter',  nbp_glo, indices_g)
  ENDIF
  ...
  IF (ok_pc) THEN
     ...
     IF (perma_peat) THEN
        ...
        CALL restput_p(rest_id_sto, 'wtpmax', nbp_glo, nvm, 1,itau_dep, &
         &                  wtpmax, 'scatter',  nbp_glo, indices_g)
