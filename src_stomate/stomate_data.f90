...
!_ ================================================================================================================================
  ...
  SUBROUTINE data (npts, lalo)
    ...
    REAL(r_std),DIMENSION (nvm)                  :: mindia
!_ ================================================================================================================================
...
!!!$      ENDIF
       ...
       IF ( is_tree(j) ) THEN
          ...
          alpha = alpha_tree
!!!qcj++ peatland
          IF (ok_dgvm_peat .AND. pipe_shrub .AND. is_shrubpeat(j)) THEN
            mindia(j)= (height_presc(j)/(10.0*8.0))**(1./ 0.55)
            dia=mindia(j)*5.0
            csa_sap = pi * dia**(1/dia_coeff(2)) / ( mass_ratio_heart_sap * dia_coeff(1) )

            bm_sapl(j,ileaf,icarbon) = csa_sap * 13.e3 / sla(j)
            !bm_sapl(j,ileaf,icarbon) = &
            !   &     ((bm_sapl_leaf(1)*216.9*(mass_ratio_heart_sap *bm_sapl_leaf(2)*sla(j)/(pi*pipe_k1)) &         
            ...
            bm_sapl(j,isapabove,icarbon) = &
               bm_sapl_sapabove * pipe_density * csa_sap * 8.0* dia **0.55
       ...
       ELSE
          ...
          IF (ok_LAIdev(j)) THEN
              ...
              !bm_sapl(j,icarbres,icarbon) = zero
          ELSE
              IF ( natural(j) .OR. is_grassland_manag(j) .AND. ( .NOT. is_mosspeat(j) )) THEN 
                 bm_sapl(j,ileaf,icarbon) = init_sapl_mass_leaf_nat / sla(j)
              ELSEIF ( is_mosspeat(j) ) THEN
                 bm_sapl(j,ileaf,icarbon) = init_sapl_mass_leaf_novasc / sla(j) !!!qcj++ peatland following Druel et al., 2017 GMD paper
              ELSE  
                 bm_sapl(j,ileaf,icarbon) = init_sapl_mass_leaf_agri / sla(j)
              ENDIF
              IF ( .NOT. is_mosspeat(j) ) THEN
                 bm_sapl(j,icarbres,icarbon) = init_sapl_mass_carbres *bm_sapl(j,ileaf,icarbon)
              ELSE
                 bm_sapl(j,icarbres,icarbon) = zero    !!!qcj++ peatland following Druel et al., 2017 GMD paper
              ENDIF
          ENDIF
       ...
       ELSE !!!qcj++ peatland following Druel et al., 2017 GMD paper
          bm_sapl(j,iroot,icarbon) = zero
          bm_sapl(j,ifruit,icarbon) = 0.05  * bm_sapl(j,ileaf,icarbon)
       ENDIF
...
!!!qcj++ peatland
          IF (ok_dgvm_peat .AND. pipe_shrub .AND. is_shrubpeat(j)) THEN
           ! maxdia(j) = ( ( pipe_tune4 / ((8.0*0.55)/(maxdia_coeff(1)**0.55)) ) &
           !    ** ( un / ( 0.55 - un ) ) ) * maxdia_coeff(2)
            maxdia(j) = ( height_presc(j)/8.0) ** (1.0/0.55)
          ELSE
