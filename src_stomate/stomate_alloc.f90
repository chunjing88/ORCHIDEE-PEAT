...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-MICT/ORCHIDEE/src_stomate/stomate_alloc.f90 $
...
!ENDJCMODIF
    ...
    !              biomass(i,j,icarbres) = biomass(i,j,icarbres)
       ...
       ELSE
           ...
           WHERE ( ( biomass(:,j,ileaf,icarbon) .GT. zero ) .AND. & 
                ...
                ( when_growthinit(:,j) .LT. reserve_time ) .AND. &
                ( .NOT. is_mosspeat(j) ) ) !!!qcj++ peatland following Druel et al., 2017 GMD paper
 ...
 !! 3. Calculate allocatable fractions of biomass production (NPP) 
    ...
    !!gmjc balance the above/below ground allocation to avoid strong unbalance
    ...
    !       IF (j.EQ.10) WRITE(numout,*) 'zd toLSR 1 ','RtoLSR(1)',RtoLSR(1),'limit_WorN(1)',limit_WorN(1),'lai_around(1,10)',lai_around(1,10),'moiavail_week(1,10)',moiavail_week(1,10),'h_nitrogen(1)',h_nitrogen(1),'t_nitrogen(1)',t_nitrogen(1),'tsoil_month(1,:)',tsoil_month(1,:),'rpc(1)',rpc(1),'soilhum_month(1,:)',soilhum_month(1,:)
           WHERE ( biomass(:,j,ileaf,icarbon) .GT. min_stomate .AND. ( .NOT. is_mosspeat(j) ) ) !!!qcj++ peatland following Druel et al., 2017 GMD paper
    ...
    !       IF (j.EQ.10) WRITE(numout,*) 'zd f_alloc 1 ','f_alloc(1,10,ileaf)',f_alloc(1,10,ileaf),'LtoLSR(1)',LtoLSR(1),'RtoLSR(1)',RtoLSR(1),'StoLSR(1)',StoLSR(1)
           DO i = 1, npts ! Loop over grid cells 
              ...
              IF ( biomass(i,j,ileaf,icarbon) .GT. min_stomate ) THEN
                 ...
                 IF ( senescence(i,j) .AND. ( .NOT. is_mosspeat(j) ) ) THEN !!!qcj++ peatland following Druel et al., 2017 GMD paper
    ...
    !                IF ( ( biomass(i,j,icarbres)*sla(j) ) .LT. 2*lai_max(j) ) THEN
                  IF ( .NOT. is_mosspeat(j) ) THEN !!!qcj++ peatland following Druel et al., 2017 GMD paper
    ...
    !ENDJCMODIF
                    ...
                    ENDIF
                  ELSE  !!!qcj++ peatland following Druel et al., 2017 GMD paper
                    f_alloc(i,j,ifruit) = f_fruit/2
                    f_alloc(i,j,ileaf) = un - f_alloc(i,j,ifruit)/2
                    f_alloc(i,j,isapabove) = zero
                    f_alloc(i,j,isapbelow) = zero
                    f_alloc(i,j,iroot) = zero
                    f_alloc(i,j,icarbres) = zero
                  ENDIF
                 ENDIF  ! Is senescent?
