...
!      IF (f_autogestion .EQ. 3) THEN
        able_grazing(:,mgraze_C3) = sr_ugb(:,mgraze_C3) * 10000.0 * 130.0 * & 
               2.0**(1.0-(sr_ugb(:,mgraze_C3)*10000.0))/1000.0*factor_able_grazing
        able_grazing(:,mgraze_C4) = sr_ugb(:,mgraze_C4) * 10000.0 * 130.0 * &
               2.0**(1.0-(sr_ugb(:,mgraze_C4)*10000.0))/1000.0*factor_able_grazing
        !set maximum threshold
        WHERE(able_grazing(:,mgraze_C3) .GT. 0.13*factor_able_grazing)
          able_grazing(:,mgraze_C3) = 0.13*factor_able_grazing
        END WHERE
...
!!!!!! gmjc for global simulation with wild animal grazing natural grassland
        able_grazing(:,mnatural_C3) = sr_wild(:,mnatural_C3) * 10000.0 * 130.0 * &
               2.0**(1.0-(sr_wild(:,mnatural_C3)*10000.0))/1000.0*factor_able_grazing
        able_grazing(:,mnatural_C4) = sr_wild(:,mnatural_C4) * 10000.0 * 130.0 * &
               2.0**(1.0-(sr_wild(:,mnatural_C4)*10000.0))/1000.0*factor_able_grazing

        WHERE (able_grazing(:,mnatural_C3) .GE. 0.13*factor_able_grazing)
          able_grazing(:,mnatural_C3) = 0.13*factor_able_grazing
        ELSEWHERE (able_grazing(:,mnatural_C3) .LT. 0.006*factor_able_grazing)
          able_grazing(:,mnatural_C3) = 0.006*factor_able_grazing
        ENDWHERE
        WHERE (able_grazing(:,mnatural_C4) .GE. 0.13*factor_able_grazing)
          able_grazing(:,mnatural_C4) = 0.13*factor_able_grazing
        ELSEWHERE (able_grazing(:,mnatural_C4) .LT. 0.006*factor_able_grazing)
          able_grazing(:,mnatural_C4) = 0.006*factor_able_grazing
        ENDWHERE
        !
        ! > 1 LSU/ha using 0.25 kgDM
...
!print *,'BM_threshold',BM_threshold,BM_threshold_turnout
    DO j=2,nvm
      IF (is_grassland_grazed(j).AND.(.NOT.is_grassland_cut(j)) .AND. &
          (.NOT. is_c4(j)) .AND. (.NOT.is_tree(j)) .AND. (.NOT.is_peat(j)))THEN
        mgraze_C3=j
      END IF
      IF (is_grassland_grazed(j).AND.(.NOT.is_grassland_cut(j)) .AND. &
          (is_c4(j)) .AND. (.NOT.is_tree(j)) .AND. (.NOT.is_peat(j)))THEN
        mgraze_C4=j
      END IF
        IF ( (.NOT.is_grassland_manag(j)) .AND.(.NOT.is_grassland_grazed(j)).AND. &
          (.NOT.is_grassland_cut(j)) .AND. (.NOT. is_c4(j)) .AND. (.NOT.is_tree(j)) &
          .AND. natural(j) .AND. (.NOT.is_peat(j)))THEN
          mnatural_C3=j
        END IF
        IF ( (.NOT.is_grassland_manag(j)) .AND.(.NOT.is_grassland_grazed(j)).AND. &
