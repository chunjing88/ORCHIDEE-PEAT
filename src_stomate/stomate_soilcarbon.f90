...
!!!qcj++ peatland
   ...
   REAL(r_std),DIMENSION(npts,nvm),INTENT(in)                  :: wtp_peat
   !! 0.2 Output variables 
    REAL(r_std), DIMENSION(npts),INTENT(out)               :: tcarbon_acro
    REAL(r_std), DIMENSION(npts),INTENT(out)               :: tcarbon_cato
    REAL(r_std), DIMENSION(npts,nvm),INTENT(out)           :: resp_acro_oxic   !!respiration of acrotelm( oxic )
    REAL(r_std), DIMENSION(npts,nvm),INTENT(out)           :: resp_acro_anoxic !!respiration of acrotelm( anoxic )
    REAL(r_std), DIMENSION(npts,nvm),INTENT(out)           :: resp_cato        !!respiration of catotelm
    REAL(r_std), DIMENSION(npts,nvm),INTENT(out)           :: litter_to_acro   !!total carbon input from litter to acrotelm
    REAL(r_std), DIMENSION(npts,nvm),INTENT(out)           :: acro_to_cato     !!carbon flux from acrotelm to catotelm
    REAL(r_std), DIMENSION(npts), INTENT(out)              :: height_cato

    !! 0.3 Modified variables
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)              ::carbon_acro
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)              ::carbon_cato
    REAL(r_std), DIMENSION(npts), INTENT(inout)                   ::height_acro

    !! 0.4 Local variables
    REAL(r_std), DIMENSION(npts,nvm)            ::B    !acrotelm oxic respiration 
    ...
    REAL(r_std), DIMENSION(npts,nvm)            ::wtd
...
!end gmjc      
      ...
      !       carbon(:,:,m) = carbon(:,:,m) + SUM( flux(:,:,:), DIM=2 )
        ...
        IF (ok_peat) THEN      
           ...
           carbon_cato(:,m)=zero
           acro_to_cato(:,m)=zero
           litter_to_acro(:,m)=zero
           resp_cato(:,m)=zero
           resp_acro_anoxic(:,m)=zero
           resp_acro_oxic(:,m)=zero
        ENDIF
     ENDIF  ! is_peat 
    ...
    IF (ok_peat) THEN
     !!! decomposition rates and catotelm formation rates are constrained by temperature
       ...
       DO k=1,npts
          DO m=2,nvm
             IF (is_peat(m)) THEN
               wtd(k,m)=wtd_min-wtp_peat(k,m) !in mm
               IF (wtd(k,m) .LE. 0.0) THEN
                 B(k,m)=1.0
               ELSE IF (wtd(k,m) .LT. height_acro(k)) THEN
                 B(k,m)=(height_acro(k)-wtd(k,m))/height_acro(k)
               ELSE IF (wtd(k,m) .GE. height_acro(k)) THEN
                 B(k,m)=0.0
               ENDIF

               carbon(k,:,m) =zero

               resp_acro_oxic(k,m)=resp_acro_oxic(k,m)+B(k,m)*KA(k)*carbon_acro(k,m)
               resp_acro_anoxic(k,m)=resp_acro_anoxic(k,m)+(1.-B(k,m))*v_ratio*KA(k)*carbon_acro(k,m)
               acro_to_cato(k,m)=acro_to_cato(k,m)+KP(k)*carbon_acro(k,m)
               resp_cato(k,m)=resp_cato(k,m)+KC(k)*carbon_cato(k,m)
      !!!litter added to default active and slow soil carbon pool are added into acrotelm
               resp_hetero_soil(k,m) = (resp_acro_oxic(k,m)+resp_acro_anoxic(k,m)+resp_cato(k,m))/dt
               litter_to_acro(k,m)=litter_to_acro(k,m)+soilcarbon_input(k,iactive,m)*dt+ &
                                     &   soilcarbon_input(k,islow,m)*dt
               carbon_acro(k,m)=carbon_acro(k,m)+litter_to_acro(k,m)- &
                               & acro_to_cato(k,m)-resp_acro_oxic(k,m)-resp_acro_anoxic(k,m)
               carbon_cato(k,m)=carbon_cato(k,m)+acro_to_cato(k,m)-resp_cato(k,m)

               tcarbon_acro(k)=tcarbon_acro(k)+carbon_acro(k,m)
               tcarbon_cato(k)=tcarbon_cato(k)+carbon_cato(k,m)
               height_acro(k)=tcarbon_acro(k)/(p_A*cf_A)
               height_cato(k)=tcarbon_cato(k)/(p_C*cf_C) 
             ENDIF
          ENDDO
       ENDDO
    ENDIF
