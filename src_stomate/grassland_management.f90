...
! Auxilar variables
    ...
    init_grassland : IF (first_call_grassland_manag) THEN
      ...
      ! 6: no anthropogenic management (grazing/cutting) but with Ndepo and wild
      ! grazing
      f_postauto = 0
      ...
      IF (N_effect .LT. 0.0 .OR. N_effect .GT. 1.0) THEN
        N_effect =0.6
      ENDIF
      ! factor to scale the WSHTOT that starting and ending the grazing
      factor_able_grazing = 1.0
      CALL getin_p('GRM_FACTOR_GRAZING',factor_able_grazing)
      ...
      DO j=2,nvm
        ...
        IF (is_grassland_manag(j) .AND. (.NOT. is_grassland_cut(j)) .AND. &
           ...
           (.NOT. is_tree(j)) .AND. (.NOT.is_peat(j))) THEN
          mauto_C3=j
        END IF
        ...
        IF ((.NOT. is_grassland_manag(j)) .AND. (.NOT. is_grassland_grazed(j)) .AND. &
           (.NOT. is_grassland_cut(j)) .AND. (.NOT. is_c4(j)) .AND. &
           (.NOT. is_tree(j)) .AND. natural(j) .AND. (.NOT.is_peat(j))) THEN
          mnatural_C3=j
        END IF
        IF ((.NOT. is_grassland_manag(j)) .AND. (.NOT. is_grassland_grazed(j)).AND. &
          (.NOT. is_grassland_cut(j)) .AND. (is_c4(j)) .AND. &
...
!          tfert(:,j,1)=90
      ...
      ! f_postauto .EQ. 6 for no management run 
      ! where there is no grazing, no fertilization, but with N deposition
      IF (f_postauto .EQ. 6) THEN
        nslurry(:,:,:) = 0.0
        nfertamm(:,:,:) = 0.0
        nfertnit(:,:,:) = 0.0
        tfert(:,:,:) = 500
