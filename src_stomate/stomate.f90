...
!! $Date: 2018-06-13 14:31:34 +0200 (Wed, 13 Jun 2018) $
!! $Revision: 5292 $
!! \n
!_ ================================================================================================================================
...
MODULE stomate
  ...
  USE defprec
  USE grid
  USE time, ONLY : one_day, one_year, dt_sechiba, dt_stomate, LastTsYear, LastTsMonth, FirstTsYear
...
!! xuhui
  ...
  PRIVATE
  PUBLIC stomate_main,stomate_clear,init_forcing, stomate_forcing_read, stomate_initialize, stomate_finalize, stomate_init_index
...
!---
  ...
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: resp_hetero_litter_ma_d
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: resp_hetero_litter_mb_d
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: resp_hetero_litter_sa_d
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: resp_hetero_litter_sb_d
...
!!!qcj++ peatland
   ...
   REAL(r_std),ALLOCATABLE,SAVE, DIMENSION(:,:)    ::resp_acro_oxic_d
   REAL(r_std),ALLOCATABLE,SAVE, DIMENSION(:,:)    ::resp_acro_anoxic_d
   REAL(r_std),ALLOCATABLE,SAVE, DIMENSION(:,:)    ::resp_cato_d
   REAL(r_std),ALLOCATABLE,SAVE, DIMENSION(:,:)    ::acro_to_cato_d
   REAL(r_std),ALLOCATABLE,SAVE, DIMENSION(:,:)    ::litter_to_acro_d
!   REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: harvest_bio

   REAL(r_std),ALLOCATABLE,DIMENSION(:,:,:)         :: wtp_pt
   REAL(r_std),ALLOCATABLE,DIMENSION(:,:)           :: wtp_daily
   REAL(r_std),ALLOCATABLE,DIMENSION(:,:)           :: wtpmax_thisyear
   REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: wtp_month
   REAL(r_std),ALLOCATABLE,DIMENSION(:,:)           :: wtp_month_max_thisyear
   REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: wtpmax_month

   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: fwet_daily
   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: liqwt_daily
   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: fwet_month
   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: liqwt_month
   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: liqwt_max
   REAL(r_std),ALLOCATABLE,DIMENSION(:)      :: liqwt_max_lastyear
   REAL(r_std),ALLOCATABLE,DIMENSION(:,:)    :: fwet_series
   INTEGER(i_std),SAVE                       :: nummonth   
   REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: npp0_cumul   !!Druel et al. 2017 GMD paper, Eq.3 d_cum   

!   INTEGER(i_std),SAVE                       :: sat_duration
   INTEGER(i_std),SAVE                       :: months_num
   REAL(r_std),SAVE                          :: frac_exudate
   REAL(r_std),ALLOCATABLE,DIMENSION(:,:)    :: root_exudate_daily
   REAL(r_std),DIMENSION(:,:,:),ALLOCATABLE     :: root_exudate_2pfcforcing

   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: deepC_peat 
!!!used in Lpj: peat_adjust
   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::carbon_save
   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: deepC_a_save !!Peat active Cdensity memory before peatland shrink
   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: deepC_s_save !!Peat active Cdensity memory before peatland shrink
   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: deepC_p_save !!Peat active Cdensity memory before peatland shrink
   REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:)   :: delta_fsave  !!Peatland area fraction memory
...
!!!qcj++ peatland
        &         carbon_acro, carbon_cato, height_acro,fwet_daily,fwet_month,fwet_series,fpeat,months_num,& 
        &         deepC_peat,carbon_save, deepC_a_save,deepC_s_save,deepC_p_save,delta_fsave,&
        &         liqwt_daily,liqwt_month,liqwt_max,wtpmax, npp0_cumul, wtp_month,wtpmax_month,wtp_year)        
...
!!!qcj++ peatland
    ALLOCATE(wtpmax_thisyear(kjpindex,nvm),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'There is an allocation variable error: ', 'wtpmax_thisyear', '')
    wtpmax_thisyear(:,:)=zero
    ALLOCATE(wtp_month_max_thisyear(kjpindex,nvm),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'There is an allocation variable error: ', 'wtp_month_max_thisyear', '')
    wtp_month_max_thisyear(:,:)=zero
...
!!!!! crop managements, xuhui
   IF (ANY(ok_LAIdev)) THEN    
    ...
    ENDIF ! ANY(ok_LAIdev)
! JC COMMENT should not be 0, the default value should be 1
!    N_limfert = zero
    N_limfert = 1.0
!!!! end crops, xuhui
   ...
   IF (ok_co2 .AND. allow_forcing_write) THEN
     ...
     !! 1.6.4.2 Allocate memory for variables containing forcing data  
         ...
         IF (is_root_prc) THEN
            ...
            CALL nccheck (NF90_CREATE (TRIM(stomate_forcing_name),NF90_SHARE,forcing_id))
            ...
            ! Add new variable
            CALL nccheck (NF90_DEF_VAR (forcing_id,'points',    r_typ,d_id(1),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'layers',    r_typ,d_id(2),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'pft',       r_typ,d_id(3),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'direction', r_typ,d_id(4),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'nneigh',    r_typ,d_id(5),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'time',      r_typ,d_id(6),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'nbparts',   r_typ,d_id(7),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'ngrnd',     r_typ,d_id(8),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'index',     r_typ,d_id(1),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'contfrac',  r_typ,d_id(1),vid) )
            CALL nccheck (NF90_DEF_VAR (forcing_id,'lalo', &
                 & r_typ,(/ d_id(1),d_id(4) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'neighbours', &
                 & r_typ,(/ d_id(1),d_id(5) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'resolution', &
                 & r_typ,(/ d_id(1),d_id(4) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'clay', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'humrel', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'litterhum', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'t2m', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'t2m_min', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'tsurf', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'tsoil', &
                 & r_typ,(/ d_id(1),d_id(2),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'soilhum', &
                 & r_typ,(/ d_id(1),d_id(2),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'precip', &
                 & r_typ,(/ d_id(1),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'gpp', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'veget', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'veget_max', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid))
            CALL nccheck (NF90_DEF_VAR (forcing_id,'lai', &
                 & r_typ,(/ d_id(1),d_id(3),d_id(6) /),vid))
            CALL nccheck (NF90_ENDDEF (forcing_id))
   ...
   !! 1.4.7 write forcing file for forcesoil
      ...
      IF ( TRIM(stomate_Cforcing_name) /= 'NONE' ) THEN
         ...
         IF ( nparan < 1 ) THEN
             ...
             CALL ipslerr_p(3, '', 'Invalid value for FORCESOIL_STEP_PER_YEAR ', 'Expected value is > 0', temp_str)
         ENDIF
...
!!!qcj++ peatland
         IF (ok_peat) THEN
             ALLOCATE(wtp_pt(kjpindex,nvm,nparan*nbyear), stat=ier)
             IF (ier /= 0) CALL ipslerr_p(3, 'stomate_initialize', 'Problem allocating wtp_pt', 'Error code=', ier)
             wtp_pt(:,:,:)=zero
         ENDIF
...
!!!qcj++ peatland
         ALLOCATE(root_exudate_2pfcforcing(kjpindex,nvm,nparan*nbyear))
         root_exudate_2pfcforcing(:,:,:) = zero
      ENDIF
...
!!!qcj++ peatland
   frac_exudate=0.0
   CALL getin_p('FRAC_EXUDATE', frac_exudate)
...
!!!qcj++ peatland
   IF ( ABS( SUM( fwet_month(:) ) ) .LT. min_stomate ) THEN
       WRITE(numout,*) 'Warning! We have to initialize the ''monthly'' fwet.'
       fwet_month(:)=fwet_daily(:)
   ENDIF
   IF (peat_occur) THEN
       IF ( ABS( SUM( fwet_series(:,:) ) ) .LT. min_stomate ) THEN
          WRITE(numout,*) 'Warning! We have to initialize the fwet_series.'
          fwet_series(:,:)=zero
       ENDIF
       IF ( ABS( SUM( liqwt_month(:) ) ) .LT. min_stomate ) THEN
          WRITE(numout,*) 'Warning! We have to initialize the ''monthly''liqwt.'
          liqwt_month(:)=liqwt_daily(:)
       ENDIF
...
!!!qcj++ peatland
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std),DIMENSION(kjpindex),INTENT(in)      :: precip_snow       !! Snow precipitation  
                                                                         !! @tex $(mm dt_stomate^{-1})$ @endtex 
!!!qcj++ peatland
    REAL(r_std),DIMENSION(kjpindex,nvm)              :: root_exudate_d
    REAL(r_std),DIMENSION (kjpindex,nslm,nvm),INTENT(in) :: shumdiag_peat
    REAL(r_std),DIMENSION(kjpindex),INTENT(in)    :: fwet_new    !!!!fraction of wetland
    ...
    REAL(r_std),ALLOCATABLE,DIMENSION(:,:,:)        :: wtp_pt_g
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)    :: mc_peat_above
    REAL(r_std),DIMENSION(kjpindex),INTENT(out)   :: peatC
    REAL(r_std),DIMENSION(kjpindex)               :: sumpeat
    REAL(r_std),DIMENSION(kjpindex,ndeep,nvm)     :: mc_peat
    REAL(r_std),DIMENSION(kjpindex,nslm)          :: mcsort_peat
    INTEGER(i_std),DIMENSION(kjpindex),INTENT(in) :: sat_duration
    INTEGER(i_std)                                ::yr, mh, dayn
    REAL(r_std)                                   ::snd
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(out)  :: veget_cov_max_adjusted
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)  :: veget_cov_max_new_peatdgvm
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm)    :: hsdeep_new   !! deep long term soil humidity profile
    REAL(r_std), DIMENSION(kjpindex,nstm),INTENT(in)   :: fpeat_map
...
!end gmjc
    ...
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: resp_hetero_litter       !! Litter heterotrophic respiration per ground area 
                                                                              ...
                                                                              !! over ::dt_sechiba?? 
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: resp_hetero_litter_ma
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: resp_hetero_litter_mb
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: resp_hetero_litter_sa
    REAL(r_std),DIMENSION(kjpindex,nvm)           :: resp_hetero_litter_sb
...
!!!qcj++ peatland
    ...
    !fbact = microactem(tdeep, frozen_respiration_func, hsdeep, kjpindex, ndeep, nvm, zz_deep, mc_peat)

!!! qcj++ peatland
!! Use peatland mc to calculate moisture inhibition function (Moyano et al., 2012)
    IF (perma_peat) THEN
        !mcsort_peat=shumdiag_peat(:,:)
        !CALL sort_ascending(kjpindex,mcsort_peat,11)
      mc_peat(:,:,:) = zero
      DO j = 1, ndeep
         IF (j .LE. nslm) THEN 
            mc_peat(:,j,:) = shumdiag_peat(:,j,:)
         ELSE
            mc_peat(:,j,:) = shumdiag_peat(:,nslm,:) 
         ENDIF
      ENDDO
      ...
      DO jv=1,nvm
        IF ( is_peat(jv) .OR. is_croppeat(jv) ) THEN
           hsdeep_new(:,:,jv)=mc_peat(:,:,jv)
        ELSE
           hsdeep_new(:,:,jv)=hsdeep(:,:,jv)
        ENDIF
      ENDDO
    ENDIF

    IF (perma_peat) THEN
       fbact = microactem(tdeep, frozen_respiration_func, hsdeep_new, kjpindex, ndeep, nvm, zz_deep)
    ELSE
...
!end gmjc
!!!qcj++ peatland
    CALL stomate_accu (do_slow, fwet_new, fwet_daily)
    CALL stomate_accu (do_slow, liqwt_ratio, liqwt_daily)
...
!gmjc
         sla_calc,do_slow, &
         resp_hetero_litter_ma,resp_hetero_litter_mb,resp_hetero_litter_sa,resp_hetero_litter_sb, &
!end gmjc
!!!qcj++ peatland
         shumdiag_peat,mc_peat_above)    
    ...
    resp_hetero_litter_ma(:,:) = resp_hetero_litter_ma(:,:) * dt_sechiba/one_day
    resp_hetero_litter_mb(:,:) = resp_hetero_litter_mb(:,:) * dt_sechiba/one_day
    resp_hetero_litter_sa(:,:) = resp_hetero_litter_sa(:,:) * dt_sechiba/one_day
    resp_hetero_litter_sb(:,:) = resp_hetero_litter_sb(:,:) * dt_sechiba/one_day

!!!qcj++ peatland
    root_exudate_d(:,:)=gpp_d(:,:)*frac_exudate
    root_exudate_daily(:,:)=gpp_daily(:,:)*frac_exudate

    ! convert from per time step to per day
    CALL xios_orchidee_send_field("resp_hetero_litter",resp_hetero_litter*one_day/dt_sechiba)
    CALL xios_orchidee_send_field("resp_hetero_litter_ma",resp_hetero_litter_ma*one_day/dt_sechiba)
    CALL xios_orchidee_send_field("resp_hetero_litter_mb",resp_hetero_litter_mb*one_day/dt_sechiba)
    CALL xios_orchidee_send_field("resp_hetero_litter_sa",resp_hetero_litter_sa*one_day/dt_sechiba)
    CALL xios_orchidee_send_field("resp_hetero_litter_sb",resp_hetero_litter_sb*one_day/dt_sechiba)

    CALL xios_orchidee_send_field("resp_hetero_litter_tot",SUM(resp_hetero_litter*one_day/dt_sechiba*veget_max,dim=2))
    CALL xios_orchidee_send_field("resp_hetero_litter_ma_tot",SUM(resp_hetero_litter_ma*one_day/dt_sechiba*veget_max,dim=2))
    CALL xios_orchidee_send_field("resp_hetero_litter_mb_tot",SUM(resp_hetero_litter_mb*one_day/dt_sechiba*veget_max,dim=2))
    CALL xios_orchidee_send_field("resp_hetero_litter_sa_tot",SUM(resp_hetero_litter_sa*one_day/dt_sechiba*veget_max,dim=2))
    CALL xios_orchidee_send_field("resp_hetero_litter_sb_tot",SUM(resp_hetero_litter_sb*one_day/dt_sechiba*veget_max,dim=2))

!!!qcj++ debug
    !CALL stomate_accu (do_slow,resp_hetero_litter, resp_hetero_litter_d)
    !CALL stomate_accu (do_slow,resp_hetero_litter_ma, resp_hetero_litter_ma_d)
    !CALL stomate_accu (do_slow,resp_hetero_litter_mb, resp_hetero_litter_mb_d)
    !CALL stomate_accu (do_slow,resp_hetero_litter_sa, resp_hetero_litter_sa_d)
    !CALL stomate_accu (do_slow,resp_hetero_litter_sb, resp_hetero_litter_sb_d)
    ...
    IF ( ok_pc ) THEN
       IF ( .NOT. do_daily_permafrost ) THEN
          ...
          CALL deep_carbcycle(kjpindex, index, itime, dt_sechiba, lalo, clay, &
               ...
               fbact, fixed_cryoturbation_depth,snowdz,snowrho,&
!!!qcj++ peatland
               deepC_peat,wtpmax,root_exudate_d)
...
!!!qcj++ peatland
    ...
    IF (ok_peat) THEN
        resp_acro_oxic_d(:,:)=resp_acro_oxic_d(:,:)+resp_acro_oxic(:,:)
        resp_acro_anoxic_d(:,:)=resp_acro_anoxic_d(:,:)+resp_acro_anoxic(:,:)
        resp_cato_d(:,:)=resp_cato_d(:,:)+resp_cato(:,:)
        litter_to_acro_d(:,:)=litter_to_acro_d(:,:)+litter_to_acro(:,:)
        acro_to_cato_d(:,:)=acro_to_cato_d(:,:)+acro_to_cato(:,:)
    ENDIF
...
!!!qcj++ peatland
!!!monthly wetland fraction
       fwet_month(:)=( fwet_month(:) * ( tau_fwet_month - dt_days ) + &
                        fwet_daily(:) * dt_days ) / tau_fwet_month
       WHERE ( ABS(fwet_month(:)) .LT. EPSILON(zero) )
             fwet_month(:) = zero
       ENDWHERE
       CALL xios_orchidee_send_field('fwet_month',fwet_month)

       IF (peat_occur) THEN
           liqwt_month(:)=( liqwt_month(:) * ( tau_liqwt_month - dt_days ) + &
                        liqwt_daily(:) * dt_days ) /tau_liqwt_month
           WHERE ( ABS(liqwt_month(:)) .LT. EPSILON(zero) )
                 liqwt_month(:) = zero
           ENDWHERE
           WHERE (liqwt_month(:) .GT. liqwt_max(:)) 
               liqwt_max(:)=liqwt_month(:)
           ENDWHERE
           CALL itau2ymds(itime, dt_sechiba, yr, mh, dayn, snd)       
           dayn = (mh-1)*30 + dayn
           IF (dayn .EQ. 2) THEN
              liqwt_max_lastyear(:)=liqwt_max(:)
           ENDIF
           CALL xios_orchidee_send_field('liqwt_month',liqwt_month)
           CALL xios_orchidee_send_field('liqwt_max_lastyear',liqwt_max_lastyear)
!!!store 30 yeras' monthly wetland fraction in fwet_series
           IF (LastTsMonth) THEN 
              nummonth=floor(real((days_since_beg+1)/30))
              nummonth=MOD(nummonth,months_num)
              IF (nummonth .EQ. zero) THEN  
                  nummonth=months_num
              ENDIF      
              fwet_series(:,1)=fwet_month(:)
              CALL sort_ascending(kjpindex,fwet_series,months_num)
           ENDIF
           DO ip = 1, kjpindex
              fpeat(ip)=fwet_series(ip,sat_duration(ip))
           ENDDO
       !    WRITE (numout,*) 'jingcheck fpeat',fpeat(:)
           CALL xios_orchidee_send_field('fpeat',fpeat)
       ENDIF
...
!!!qcj++ peatland
             CALL calc_wtpmax(kjpindex,dayn,wtpmax,wtp_daily,wtpmax_thisyear,wtpmax_month,wtp_month,wtp_month_max_thisyear,LastTsYear,veget_cov_max) !!wtpmax save maximum wtp in the previous year
...
!!!qcj++ peatland
                  deepC_peat,wtpmax,root_exudate_daily)
...
!!!qcj++ peatland
             DO ip = 1, kjpindex
                ...
                sumpeat(ip)=zero
                DO j = 2, nvm
                   IF (natural(j) .AND. is_peat(j)) THEN
                      sumpeat(ip)=sumpeat(ip)+veget_cov_max(ip,j)
                      peatC(ip)=peatC(ip) + SUM(deepC_peat(ip,:,j))*veget_cov_max(ip,j) !!!peatC: g/m2
                   ENDIF  
                ENDDO
                IF ( sumpeat(ip) .GT. zero )THEN
                   peatC(ip)=peatC(ip)/sumpeat(ip)
                ENDIF 
             ENDDO
             peatC=peatC/1000.0  !!!kg/m2
          ENDIF
       ENDIF
...
!!!qcj++ peatland
                root_exudate_2pfcforcing(:,:,iatt)=root_exudate_2pfcforcing(:,:,iatt)+root_exudate_daily(:,:)
          ENDIF
...
!     CALL xios_orchidee_send_field("resp_hetero_soil",resp_hetero_soil)
!     CALL xios_orchidee_send_field("resp_hetero_litter",resp_hetero_litter_d/dt_sechiba*one_day)
!     CALL xios_orchidee_send_field("resp_hetero_litter_ma",resp_hetero_litter_ma_d/dt_sechiba*one_day)
!     CALL xios_orchidee_send_field("resp_hetero_litter_mb",resp_hetero_litter_mb_d/dt_sechiba*one_day)
!     CALL xios_orchidee_send_field("resp_hetero_litter_sa",resp_hetero_litter_sa_d/dt_sechiba*one_day)
!     CALL xios_orchidee_send_field("resp_hetero_litter_sb",resp_hetero_litter_sb_d/dt_sechiba*one_day)

!     CALL xios_orchidee_send_field("resp_hetero_soil_tot",SUM(resp_hetero_soil*veget_max,dim=2))
!     CALL xios_orchidee_send_field("resp_hetero_litter_tot",SUM(resp_hetero_litter_d/dt_sechiba*one_day*veget_max,dim=2))
!     CALL xios_orchidee_send_field("resp_hetero_litter_ma_tot",SUM(resp_hetero_litter_ma_d/dt_sechiba*one_day*veget_max,dim=2))
!     CALL xios_orchidee_send_field("resp_hetero_litter_mb_tot",SUM(resp_hetero_litter_mb_d/dt_sechiba*one_day*veget_max,dim=2))
!     CALL xios_orchidee_send_field("resp_hetero_litter_sa_tot",SUM(resp_hetero_litter_sa_d/dt_sechiba*one_day*veget_max,dim=2))
!     CALL xios_orchidee_send_field("resp_hetero_litter_sb_tot",SUM(resp_hetero_litter_sb_d/dt_sechiba*one_day*veget_max,dim=2))
...
9010  FORMAT(A52,F17.14)
       ...
       evapot_daily = zero
!!!qcj++ peatland
       root_exudate_daily (:,:) = zero
       resp_acro_oxic_d(:,:)=zero
       resp_acro_anoxic_d(:,:)=zero
       resp_cato_d(:,:)=zero
       acro_to_cato_d(:,:)=zero
       litter_to_acro_d(:,:)=zero
       wtp_daily(:,:)=zero
       fwet_daily(:)=zero
       liqwt_daily(:)=zero
...
!!!qcj++ peatland
         IF (ok_peat) THEN
             ALLOCATE(wtp_pt_g(nbp_glo,nvm,nparan*nbyear))
         ENDIF
...
!!!qcj++ peatland
         IF (ok_peat) THEN
             CALL nccheck(NF90_DEF_VAR (Cforcing_id,'wtp_pt',r_typ, &
...
!!!qcj++ peatland
         IF (ok_peat) THEN
             CALL nccheck(NF90_INQ_VARID (Cforcing_id,'wtp_pt',vid))
             CALL nccheck(NF90_PUT_VAR (Cforcing_id,vid, wtp_pt_g ))
         ENDIF
...
!!!qcj++ peatland
            root_exudate_2pfcforcing(:,:,iatt) = &
                 root_exudate_2pfcforcing(:,:,iatt)/REAL(nforce(iatt),r_std)
...
!!!qcj++ peatland
            root_exudate_2pfcforcing(:,:,iatt) = zero
...
!!qcj++ peatland
    !ALLOCATE (harvest_bio(kjpindex,nvm), stat=ier)
    !l_error = l_error .OR. (ier /= 0)
    !IF (l_error) THEN
    !   WRITE(numout,*) 'Memory allocation error for harvest_bio. We stop. We need kjpindex*nvm words',kjpindex,nvm
    !   STOP 'stomate_init'
    !ENDIF
...
!   ALLOCATE (soilcarbon_input_daily(kjpindex,ncarb,nvm), stat=ier)
   ...
   l_error = l_error .OR. (ier.NE.0)
!!!qcj++peatland
   ALLOCATE (root_exudate_daily(kjpindex,nvm), stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE (wtp_daily(kjpindex,nvm), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ...
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(resp_acro_oxic_d(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(resp_acro_anoxic_d(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(resp_cato_d(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(litter_to_acro_d(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(acro_to_cato_d(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(fwet_daily(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(fwet_month(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(liqwt_month(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(liqwt_max(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(liqwt_max_lastyear(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE (carbon_save(kjpindex,ncarb, nvm), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ALLOCATE (deepC_a_save(kjpindex, ndeep), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ALLOCATE (deepC_s_save(kjpindex, ndeep), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ALLOCATE (deepC_p_save(kjpindex, ndeep), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ALLOCATE (delta_fsave(kjpindex), stat=ier)
   l_error = l_error .OR. (ier.NE.0)
   ALLOCATE(liqwt_daily(kjpindex),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(wtp_month(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ALLOCATE(wtpmax_month(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)

   months_num=360
   CALL getin_p('MONTHS_NUM',months_num)
   ALLOCATE(fwet_series(kjpindex,months_num),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   ...
   ALLOCATE(npp0_cumul(kjpindex,nvm),stat=ier)
   l_error = l_error .OR. (ier /= 0)
   IF (l_error) THEN
      WRITE(numout,*) 'Memory allocation error for npp0_cumul. We stop. We need kjpindex*nvm words',kjpindex, nvm
      STOP 'stomate_init'
   ENDIF
...
!!!qcj++ peatland
    IF (ALLOCATED(root_exudate_daily)) DEALLOCATE(root_exudate_daily)
    ...
    IF (ALLOCATED(resp_acro_oxic_d)) DEALLOCATE(resp_acro_oxic_d)
    IF (ALLOCATED(resp_acro_anoxic_d)) DEALLOCATE(resp_acro_anoxic_d)
    IF (ALLOCATED(resp_cato_d)) DEALLOCATE(resp_cato_d)
    IF (ALLOCATED(litter_to_acro_d)) DEALLOCATE(litter_to_acro_d)
    IF (ALLOCATED(acro_to_cato_d)) DEALLOCATE(acro_to_cato_d)
    IF (ALLOCATED(wtp_daily)) DEALLOCATE(wtp_daily)
    IF (ALLOCATED(wtp_pt)) DEALLOCATE(wtp_pt)
    IF (ALLOCATED(fwet_daily)) DEALLOCATE(fwet_daily)
    IF (ALLOCATED(fwet_month)) DEALLOCATE(fwet_month)
    IF (ALLOCATED(liqwt_month)) DEALLOCATE(liqwt_month)
    IF (ALLOCATED(liqwt_max)) DEALLOCATE(liqwt_max)
    IF (ALLOCATED(liqwt_max_lastyear)) DEALLOCATE(liqwt_max_lastyear)
    IF (ALLOCATED(fwet_series)) DEALLOCATE(fwet_series)
    IF ( ALLOCATED (deepC_peat)) DEALLOCATE(deepC_peat)
    IF ( ALLOCATED (carbon_save)) DEALLOCATE(carbon_save)
    IF ( ALLOCATED (deepC_a_save)) DEALLOCATE(deepC_a_save)
    IF ( ALLOCATED (deepC_s_save)) DEALLOCATE(deepC_s_save)
    IF ( ALLOCATED (deepC_p_save)) DEALLOCATE(deepC_p_save)
    IF ( ALLOCATED (delta_fsave)) DEALLOCATE(delta_fsave)
    IF (ALLOCATED(liqwt_daily)) DEALLOCATE(liqwt_daily)
    IF (ALLOCATED(wtpmax_thisyear)) DEALLOCATE(wtpmax_thisyear)
    IF (ALLOCATED(npp0_cumul)) DEALLOCATE(npp0_cumul)
    IF (ALLOCATED(wtp_month)) DEALLOCATE(wtp_month) 
    IF (ALLOCATED(wtp_month_max_thisyear)) DEALLOCATE(wtp_month_max_thisyear)
    IF (ALLOCATED(wtpmax_month)) DEALLOCATE(wtpmax_month)
...
!!!qcj++ peatland
   ! IF ( ALLOCATED (harvest_bio)) DEALLOCATE (harvest_bio)
...
!_ ================================================================================================================================   
    ...
    IF (ALL(assim_param(:,:,:)==val_exp)) THEN
       ...
       CALL vmax (kjpindex, dt_0, leaf_age_tmp, leaf_frac_tmp, vcmax, N_limfert,humrel_month) !!!qcj++ peatland, following Druel et al. 2017 GMD paper,add humrel_month for dessication impact 
...
!!
!================================================================================================================================
!! SUBROUTINE   :sort_ascending
!!
!>\BRIEF
!!
!! DESCRIPTION  : sort the array into ascending
!!
!!
!!
!! RECENT CHANGE(S) : None
...
!================================================================================================================================
  ...
  SUBROUTINE sort_ascending(kjpindex,array,months_num)
    ...
    !! 0.1 Input variables
       ...
       INTEGER(i_std),INTENT(in)                       :: months_num
       REAL(r_std), DIMENSION(kjpindex,months_num), INTENT(INOUT) :: array
...
!================================================================================================================================
  ...
  DO i=1,kjpindex
     DO j=1,months_num-1
        ...
        location=j
        DO k=j+1,months_num
           IF (array(i,k) .LT. minimum) THEN
              minimum=array(i,k)
              location=k
           ENDIF
        ENDDO
        temp=array(i,j)
        array(i,j)=array(i,location)
        array(i,location)=temp
     ENDDO
  ENDDO
...
!================================================================================================================================
!! SUBROUTINE   :calc_wtpmax
!!
!>\BRIEF
!!
!! DESCRIPTION  : calculate maximum water table depth, to constrain rooting depth
!!
!!
!! RECENT CHANGE(S) : None
!!
...
!_
!================================================================================================================================
  SUBROUTINE calc_wtpmax(kjpindex,dayno,wtpmax,wtp_daily,wtpmax_thisyear,wtpmax_month,wtp_month,wtp_month_max_thisyear,LastTsYear,veget_cov_max)
    INTEGER(i_std),INTENT(in)                       :: kjpindex          !!Domain size - terrestrial pixels only (unitless)
    INTEGER(i_std), INTENT(in)                      :: dayno !! number of the day in the current year
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)      :: wtp_daily
    LOGICAL, INTENT(in)                             :: LastTsYear
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: wtpmax
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: wtpmax_thisyear

    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)      :: wtp_month
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: wtpmax_month
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)   :: wtp_month_max_thisyear
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)      :: veget_cov_max
    REAL(r_std),DIMENSION(kjpindex)                     :: peatlandveg
    INTEGER(i_std)                                  :: ip,ji, jv

    WHERE ( wtp_daily(:,:) .GT. wtpmax_thisyear(:,:) )
       wtpmax_thisyear(:,:) = wtp_daily(:,:)
    ENDWHERE
  
    WHERE (wtp_month(:,:) .GT. wtp_month_max_thisyear(:,:) )
       wtp_month_max_thisyear(:,:)=wtp_month(:,:)
    ENDWHERE


   IF (LastTsYear) THEN
      wtpmax(:,:) = wtpmax_thisyear(:,:)
      wtpmax_thisyear(:,:)=wtp_daily(:,:)

      wtpmax_month(:,:)=wtp_month_max_thisyear(:,:)
      wtp_month_max_thisyear(:,:)=wtp_month(:,:)    
   ENDIF

  END SUBROUTINE calc_wtpmax
