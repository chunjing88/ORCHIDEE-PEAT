...
!_ ================================================================================================================================
  ...
  SUBROUTINE Get_harvest_matrix(npts,veget_max,newvegfrac,harvest_matrix, &
    ...
    model_total = zero
    DO ipts = 1,npts
       model_total = model_total + total_abwood_harvest(ipts)
    ENDDO
    WRITE(numout,*) 'model_total: ',model_total

    CALL allreduce_sum(input_total, input_total_allproc)
    WRITE(numout,*) 'Regional total input wood harvest,', input_total_allproc
    IF ( input_total_allproc .GT. min_stomate) THEN
      ...
      WRITE(numout,*) 'Initial ratio is: ',ratio
      WRITE(numout,*) 'Initial regional total output wood harvest,', model_total_allproc
    ENDIF
    ...
    DO WHILE ( (ABS(ratio-1) .GT. 0.01) .AND. (ratio.GT.min_stomate) .AND. num .LT. 20)
      ...
      IF ( input_total_allproc .GT. min_stomate) THEN
         CALL allreduce_sum(model_total, model_total_allproc)
         ratio = model_total_allproc/input_total_allproc
         WRITE(numout,*) 'Get_harvest_matrix:: ratio = ',ratio,'model_total_allproc,',model_total_allproc
      ENDIF
