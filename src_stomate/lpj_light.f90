...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-MICT/ORCHIDEE/src_stomate/lpj_light.f90 $
...
!gmjc
       sla_calc,fpeat_map,wettile_dgvm)
!end gmjc
...
!!!qcj++ peatland
    ...
    REAL(r_std)                                            :: fpc_dec                  !! Relative change of number of individuals 
                                                                                       !! for trees
!!!qcj++ peatland
   REAL(r_std)                                            :: sumfpc_shrub
   REAL(r_std)                                            :: sumdelta_fpc_shrub
!_ ================================================================================================================================
...
!ENDJCMODIF
 ...
 !! 4. Write history files
  ...
  ELSE
    ...
    IF (printlev>=3) WRITE(numout,*) 'Entering light, ok_dgvm_peat'
...
!! 2. Light competition in DGVM, on 'peatland'
    fpc_peat(:,:) = zero
    light_death(:,:) = zero
    survive(:) = un
       ...
       !! 2.4.1 Dense canopy
           IF ( sumfpc(i) .GT. fpc_crit ) THEN
             ...
             DO j = 2,nvm
               IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
                 IF ( annual_increase ) THEN
                    deltafpc(j) = MAX( (fpc_peat(i,j)-maxfpc_lastyear(i,j)),  zero )
                 ELSE
                    deltafpc(j) = MAX( (fpc_peat(i,j)-veget_lastlight(i,j)),  zero )
                 ENDIF
               ENDIF
             ENDDO
             !! 2.4.1.2 Default survival
             ...
             DO j = 2,nvm ! loop over #PFTs
              ...
              IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
                ...
                IF ( is_tree(j) ) THEN
                      ...
                      ! total woody fpc
                    sumfpc_wood = sumfpc_wood + fpc_peat(i,j)

                      ! how much did the woody fpc increase
                    sumdelta_fpc_wood = sumdelta_fpc_wood + deltafpc(j)

                      ! which woody pft is preponderant
                    IF ( fpc_peat(i,j) .GT. maxfpc_wood ) THEN

                       optpft_wood = j

                       maxfpc_wood = fpc_peat(i,j)
                ...
                ELSE
                      !! 2.4.1.3.1.2 Grasses
                      ! total (natural) grass fpc
                   sumfpc_grass = sumfpc_grass + fpc_peat(i,j)

                ENDIF   ! tree or grass

                IF ( is_shrubpeat(j) ) THEN
                   sumfpc_shrub = sumfpc_shrub + fpc_peat(i,j)
             ...
             DO j = 2,nvm ! Loop over #PFTs
                ...
                ! only present and natural pfts compete
               IF ( PFTpresent(i,j) .AND. natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
                ...
                IF ( is_tree(j) ) THEN
                      ...
                      ! The reduction rate is proportional to the ratio deltafpc/fpc.
                           ! Druel et al. 2019
                           ! Shrubs have an intermediate height and are assumed
                           ! to outcompete both herbaceous and moss vegetation
                           ! for light. Therefore, if the woody vegetation FPC
                           ! defined by shrubs and trees reaches over fpc_crit,
                           ! shrubs are dominated by trees and their fractional cover is reduced 
                           ! until total woody FPC does not exceed fpc_crit. 
 
                      IF ( (sumfpc_wood .GE. fpc_crit) .AND. (fpc_peat(i,j) .GT. min_stomate) .AND. &
                           (sumdelta_fpc_wood .GT. min_stomate) ) THEN
                           IF ( sumfpc_shrub .GT. min_stomate ) THEN
                              IF ( (sumfpc_wood - sumfpc_shrub) .LT. fpc_crit ) THEN
                                 IF ( is_tree(j) .AND. (.NOT. is_shrubpeat(j)) )THEN
                                    reduct = zero
                                 ELSE !is_shrubpeat(j)
                                   IF (sumdelta_fpc_shrub .GT. min_stomate)  THEN
                                    reduct = un - MIN((fpc_peat(i,j)-(sumfpc_wood-fpc_crit) &
                                      *deltafpc(j)/sumdelta_fpc_shrub)/fpc_peat(i,j), un )
                                   ELSE
                                    reduct = un - MIN((fpc_peat(i,j)-(sumfpc_wood-fpc_crit) &
                                      *fpc_peat(i,j)/sumfpc_shrub)/fpc_peat(i,j), un )
                                   ENDIF
                                 ENDIF
                              ELSE
                                 IF (is_shrubpeat(j)) THEN
                                   reduct = un
                                 ELSE
                                    IF ( (sumdelta_fpc_wood-sumdelta_fpc_shrub) .GT. min_stomate ) THEN
                                      reduct = un - MIN((fpc_peat(i,j)-(sumfpc_wood-sumfpc_shrub-fpc_crit) &
                                      *deltafpc(j)/(sumdelta_fpc_wood-sumdelta_fpc_shrub))/fpc_peat(i,j), un )
                                    ELSE
                                      reduct = zero
                                    ENDIF  
                                 ENDIF  
                           ...
                           ELSE 
                             reduct = un - MIN((fpc_peat(i,j)-(sumfpc_wood-fpc_crit) &
                               * deltafpc(j)/sumdelta_fpc_wood)/fpc_peat(i,j), un )
                           ENDIF
                      ELSE      
                         ! tree fpc didn't icrease or it started from nothing
                           reduct = zero
                ...
                ELSE
                      ...
                      IF(sumfpc_grass .GE. un-MIN(fpc_crit,sumfpc_wood).AND. &
                         ...
                         fpc_dec = (sumfpc_grass - un + MIN(fpc_crit,sumfpc_wood))*fpc_peat(i,j)/sumfpc_grass
       ...
       !! 2.5 Recalculate fpc for natural PFTs
         ...
         DO j = 2,nvm ! loop over #PFT
          ...
          !! 2.5.1 Natural PFTs
           ...
           ENDIF  !  natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j)
...
!! 4. Write history files
    ...
    IF (printlev>=4) WRITE(numout,*) 'Leaving light, ok_dgvm_peat'
