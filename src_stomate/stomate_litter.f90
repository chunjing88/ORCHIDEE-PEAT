...
!!!qcj++ peatland
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)               :: resp_hetero_litter_ma
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)               :: resp_hetero_litter_mb
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)               :: resp_hetero_litter_sa
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)               :: resp_hetero_litter_sb
    ...
    REAL(r_std), DIMENSION(npts,nvm,nbpools), INTENT(out)         :: VectorB          !! Vector containing the litter increase per
                                                                                      ...
                                                                                      !! @tex $(gC m^{-2})$ @endtex
!!!qcj++ peatland
    REAL(r_std),DIMENSION (npts,nslm,nvm),INTENT(in)                :: shumdiag_peat
    REAL(r_std), DIMENSION(npts,nlevs,nvm)                          :: control_moist_peat
    REAL(r_std), DIMENSION(npts,nvm),INTENT(in)                     :: mc_peat_above
    REAL(r_std), DIMENSION(npts,nvm)                            :: mc_peat_below
...
!_ ================================================================================================================================
  ...
  !! 1. Initialisations of the different fields during the first call of the routine 
    ...
    resp_hetero_litter_ma(:,:) = zero
    resp_hetero_litter_mb(:,:) = zero
    resp_hetero_litter_sa(:,:) = zero
    resp_hetero_litter_sb(:,:) = zero
...
!!!qcj++ peatland : use water content instead of relative hum
    IF (perma_peat) THEN
       DO j=1,nvm
         IF (is_peat(j) .OR. is_croppeat(j)) THEN   
           control_moist_peat(:,iabove,j)=control_moist_func_peat(npts, mc_peat_above(:,j))
         ENDIF 
       ENDDO
    ENDIF

    IF (agri_peat) THEN
!!Peatland drainage, litter either be removed and emitted
!!to the atmosphere immediately or decomposing under aerobic conditions
!!here parameterized as no moisture constraint on decomposition
       DO j=1,nvm 
          IF ( is_croppeat(j) ) THEN
             control_moist_peat(:,iabove,j)=1.0 !control_moist_func_peat(npts, mc_peat_above(:,j))
          ENDIF 
       ENDDO
    ENDIF
    !
    ...
    IF (perma_peat) THEN
      ...
      DO j=1,nvm
        IF ( is_peat(j) .OR. is_croppeat(j) ) THEN
          DO l = 1, nslm !Loop over soil levels
             mc_peat_below(:,j) = &
             mc_peat_below(:,j)+shumdiag_peat(:,l,j)  * rpc(:) * &
             ( EXP( -z_soil(l-1)/z_decomp ) - EXP( -z_soil(l)/z_decomp ) )
          ENDDO
          control_moist_peat(:,ibelow,j)=control_moist_func_peat(npts, mc_peat_below(:,j))
        ENDIF 
      ENDDO   
    ENDIF
...
!!!qcj++ peatland
          IF (perma_peat) THEN
             IF ( is_peat(m) )  THEN
               fd(:) = dt/litter_tau(istructural) * &
                  control_temp(:,l) * control_moist_peat(:,l,m) * exp(-litter_struct_coef * lignin_struc(:,m,l) )
             ELSEIF ( is_croppeat(m) .AND. (.NOT. is_c4(m)) ) THEN
               fd(:) = dt/litter_tau(istructural) * &
                  control_temp(:,l) * control_moist_peat(:,l,m) * exp(-litter_struct_coef * lignin_struc(:,m,l) )*flux_tot_coeff(1)
             ELSEIF ( is_croppeat(m) .AND.  is_c4(m)  ) THEN
               fd(:) = dt/litter_tau(istructural) * &
                  control_temp(:,l) * control_moist_peat(:,l,m) * exp(-litter_struct_coef * lignin_struc(:,m,l) )*flux_tot_coeff(2)
             ELSE
               fd(:) = dt/litter_tau(istructural) * &
      ...
      ! time step and avoid some constructions that could create bug during future developments.
          ...
          if (l==iabove) resp_hetero_litter_sa(:,m)=resp_hetero_litter_sa(:,m) + &
             & (1.-frac_soil(istructural,iactive,l)) * qd(:,icarbon) *( 1. - lignin_struc(:,m,l))/ dt
          if (l==ibelow) resp_hetero_litter_sb(:,m)=resp_hetero_litter_sb(:,m) + &
             & (1.-frac_soil(istructural,iactive,l)) * qd(:,icarbon) *( 1. - lignin_struc(:,m,l))/ dt
...
!!!qcj++ peatland
          ...
          IF (perma_peat) THEN
             IF ( is_peat(m) )  THEN
               fd(:) = dt/litter_tau(imetabolic) * control_temp(:,l) * control_moist_peat(:,l,m)
             ELSEIF ( is_croppeat(m) .AND. (.NOT. is_c4(m)) ) THEN
               fd(:) = dt/litter_tau(imetabolic) * control_temp(:,l) * control_moist_peat(:,l,m)*flux_tot_coeff(1)
             ELSEIF ( is_croppeat(m) .AND.  is_c4(m) ) THEN
               fd(:) = dt/litter_tau(imetabolic) * control_temp(:,l) * control_moist_peat(:,l,m)*flux_tot_coeff(2)
             ELSE
      ...
      ! time step and avoid some constructions that could create bug during future developments.
          ...
          if (l==iabove) resp_hetero_litter_ma(:,m)=resp_hetero_litter_ma(:,m) +&
              &( 1. - frac_soil(imetabolic,iactive,l) ) * qd(:,icarbon) / dt
          if (l==ibelow) resp_hetero_litter_mb(:,m)=resp_hetero_litter_mb(:,m) +&
              &( 1. - frac_soil(imetabolic,iactive,l) ) * qd(:,icarbon) / dt
...
!================================================================================================================================
    ...
    DO ii=1,npts
       mc_ind = MIN(45, MAX(1, INT(mc_peat(ii)/0.02)+1))
