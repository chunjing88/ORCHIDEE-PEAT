...
!! $Date: 2017-07-17 12:59:54 +0200 (Mon, 17 Jul 2017) $
!! $Revision: 4507 $
!! \n
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE gap (npts, dt, &
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)            :: Kwt
...
!             WHERE ( PFTpresent(:,j) .AND. ( npp_longterm(:,j) .LE. min_stomate ) ) !npp_longterm_init ) )
 ...
 !! 3. Write to history files
  ...
  ELSE
    ...
    IF (printlev>=3) WRITE(numout,*) 'Entering gap, ok_dgvm_peat:','peat PFTs use growth efficiency, non-peat PFTs use constant mortality',lpj_gap_const_mort

    !WRITE (numout,*) 'QCJ check wt_mortality,',wt_mortality(:)
    mortality(:,:) = zero
 ...
 !! 1. Tree mortality
       IF ( is_tree(j) ) THEN
          ...
          ELSE  ! .NOT. lpj_gap_const_mort
             ...
             !! 1.2 Use constant mortality accounting for the residence time of
             !each tree PFT
             WHERE ( PFTpresent(:,j) )
 ...
 !! 2. Grasses mortality
     ...
     ELSEIF (natural(j) .AND. is_peat(j) )  THEN
!!!peat PFTs use growth efficiency to calculate tree mortality
       IF ( is_tree(j) ) THEN
          WHERE ( PFTpresent(:,j) .AND. ( lm_lastyearmax(:,j) .GT. min_stomate ) )
             delta_biomass(:) = MAX( npp_longterm(:,j) - ( turnover_longterm(:,j,ileaf,icarbon) + &
                  turnover_longterm(:,j,iroot,icarbon) + turnover_longterm(:,j,ifruit,icarbon) + &
          ...
          !WHERE (t2m_daily(:) .GT. ZeroCelsius+5.0)       
             mortality(:,j) = MIN(un, MAX(zero, (un-Kwt(:,j))*wt_mortality(j)+mortality(:,j) ))
          !ENDWHERE
 ...
 !! 2. Grasses mortality
          ...
          !WHERE (t2m_daily(:) .GT. ZeroCelsius+5.0)     
          !WRITE (numout,*) 'QCJ check PFT,',j,'mortality before,',mortality(:,j),'Kwt',Kwt(:,j),'wt_mortality',wt_mortality 
            mortality(:,j) = MIN(un, MAX(zero, (un-Kwt(:,j))*wt_mortality(j)+mortality(:,j) ))
          !ENDWHERE 
       ...
       ENDIF !is_tree or grass

     ENDIF  !natural, is_peat
    ...
    IF (printlev>=4) WRITE(numout,*) 'Leaving gap, ok_dgvm_peat'
