...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-MICT/ORCHIDEE/src_stomate/lpj_pftinout.f90 $
...
!!!qcj++ peatland
    ...
    REAL(r_std), DIMENSION(npts,nstm), INTENT(out)                 :: avail_tree      !! Space availability for trees (unitless)   
    REAL(r_std), DIMENSION(npts,nstm), INTENT(out)                 :: avail_grass     !! Space availability for grasses (unitless)      
    ...
    INTEGER(i_std)                                            :: i,j,m,jst       !! indices      
...
!ENDJCMODIF
    ...
    DO jst=1,nstm
      avail_grass(:,jst) = MAX( ( un - sumfrac_wood(:) ), min_avail )
      avail_tree(:,jst) = MAX( ( fpc_crit - sumfrac_wood(:) ), min_avail )   
    ENDDO
  ...
  !! 6. Introduce PFTs
    ...
    DO j = 2,nvm ! Loop over # PFTs
       ...
       IF ( natural(j) .AND. .NOT. pasture(j)) THEN
          ...
          IF ( is_tree(j) ) THEN
             avail(:) = avail_tree(:,jst)
          ELSE
             avail(:) = avail_grass(:,jst)
          ENDIF
  ...
  ELSE
    ...
    IF (printlev>=3) WRITE(numout,*) 'Entering pftinout, ok_dgvm_peat=T'
...
!!Agicultural PFTs
  ...
  !! 2. Total woody fpc and space avaibility on 'peatland'

    !! 2.1 peatland area in grid -> fpeat_map

    !! 2.2 Total woody fractional plant cover, on 'peatland'
         sumfrac_wood(:) = zero
         DO j = 2,nvm ! Loop over # PFTs
            IF ( natural(j).AND. is_tree(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
               WHERE(fpeat_map(:,jst).GT.min_stomate)
                   sumfrac_wood(:) = sumfrac_wood(:) + cn_ind(:,j) * ind(:,j) / fpeat_map(:,jst) &
                     * ( un - exp( - lm_lastyearmax(:,j) * sla_calc(:,j) * ext_coeff(j) ) )
                !lai changed to lm_last
               ENDWHERE
            ENDIF
    ...
    !! 2.3 Space availability, for grass on 'peatland', and tree on 'peatland'
         DO i = 1, npts ! Loop over # pixels - domain size
           avail_grass(i,jst) = MAX( ( un - sumfrac_wood(i) ), min_avail )
           avail_tree(i,jst) = MAX( ( fpc_crit - sumfrac_wood(i) ), min_avail )
         ENDDO
  ...
  !! 5 Eliminate PFTs
       ...
       ! only for natural PFTs, on peatland
  ...
  !! 6. Introduce PFTs, on peatland
         ...
         DO j = 2,nvm ! Loop over # PFTs
          ...
          ! space availability for this PFT
            IF ( is_tree(j) ) THEN
!!flooded area cannot grow boreal tree and shrub
              IF ( (.NOT. (leaf_tab(j)==1 .AND. pheno_type(j)==1)) .AND. &
                 (.NOT. (leaf_tab(j)==1 .AND. pheno_type(j)==3)) ) THEN
                 avail(:)= MIN(avail_tree(:,jst),un-fwet_month(:))
              ELSE
                 avail(:) = avail_tree(:,jst)
              ENDIF
            ELSE
          ...
          !! 6.1 Check if PFT not present but (adapted and regenerative)
            ...
            DO i = 1, npts ! Loop over # pixels - domain size
             ...
             IF ( .NOT. PFTpresent(i,j) .AND. &
                  ( adapted(i,j) .GT. adapted_crit ) .AND. &
                  ( regenerate(i,j) .GT. regenerate_crit )  .AND. (fpeat_map(i,jst) .GT. zero) .AND. (pref_soil_veg(j)==jst) ) THEN

                ! Seed are available nearby
                IF ( need_adjacent(i,j) ) THEN

                   !! 6.1.1 Climate allows introduction of the PFT but
                   !dispersion requires the
                   !        presence of seeds. Seed are considered available if
                   !        at least one
                   !        neighbouring pixel is entirely invaded by the PFT.
          ...
          !      introduction, as introduction might depend on season.
            ...
            ENDWHERE
          !! 6.3 Introduce that PFT where possible
          !      "can_introduce" means that it either exists in neighbouring
          !      grid boxes
          !      or that we do not look at neighbours, that it has not been
          !      eliminated
          !      lately, and, of course, that the climate is good for that PFT.
            IF (natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. is_mosspeat(j) .AND. (pref_soil_veg(j)==jst) ) THEN

              WHERE ( can_introduce(:) )
                PFTpresent(:,j) = .TRUE.
                senescence(:,j) = .FALSE.
                ind(:,j) = ind_0_estab * (dt/one_year) * avail(:)
              ENDWHERE
            ELSEIF(natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND.(pref_soil_veg(j)==jst) )THEN
             ...
             ! introduce at least a few saplings, even if canopy is closed
             ! initial density of individuals (ind_0) = 0.02, see
             ! 'stomate_constant.f90'
                ind(:,j) = ind_0 * (dt/one_year) * avail(:)
              ENDWHERE
            ENDIF
          ...
          !! 6.4 Expansion of the PFT within the grid box
          !      PFT expansion/dispersion to a new grid box should not be
          !      confused with
          !      expansion in areal coverage
            IF ( treat_expansion ) THEN
  ...
  !! 7. If a PFT has been present once in a grid box, we suppose that it will survive
    ...
    IF (printlev>=4) WRITE(numout,*) 'Leaving pftinout, ok_dgvm_peat=T'

  ENDIF !!!IF (.NOT. ok_dgvm_peat)
