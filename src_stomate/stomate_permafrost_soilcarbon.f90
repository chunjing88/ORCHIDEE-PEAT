...
!================================================================================================================================
  ...
  SUBROUTINE deep_carbcycle(kjpindex, index, itau, time_step, lalo, clay, &
       ...
       snowdz,snowrho,& 
!!!qcj++ peatland
       deepC_peat,wtpmax,root_exudate)
...
!!!qcj++ peatland
    REAL(r_std), DIMENSION(kjpindex,nvm),INTENT(in)           :: root_exudate
    ...
    REAL(r_std), DIMENSION (kjpindex,nvm),INTENT(in)          :: wtpmax
    INTEGER(i_std), DIMENSION (kjpindex,nvm)                  :: wtpmax_ind
    !! 0.4 Local variables
    ...
    REAL(r_std), DIMENSION(kjpindex,ncarb,ndeep)      :: deltaC1_pftmean
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm)  :: deltaC_as,deltaC_ap,deltaC_sa,deltaC_sp,deltaC_pa,deltaC_ps  !! outflux to other pools
    REAL(r_std), DIMENSION(kjpindex,ndeep)      :: deltaC_as_pftmean,deltaC_ap_pftmean
    REAL(r_std), DIMENSION(kjpindex,ndeep)      :: deltaC_sa_pftmean,deltaC_sp_pftmean
    REAL(r_std), DIMENSION(kjpindex,ndeep)      :: deltaC_pa_pftmean,deltaC_ps_pftmean
    REAL(r_std), DIMENSION(kjpindex,ncarb,ndeep,nvm)  :: dc_litter_z
    REAL(r_std), DIMENSION(kjpindex,ncarb,ndeep)      :: dc_litter_z_pftmean
...
!!!qcj++ peatland
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm) :: QCJ_tmp
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm) :: soilcin_active !!active litter input,  for each layer
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm)     :: foc   !! active peat litter input
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm)     :: Kprim
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm)  :: root_exudate_z
!!!qcj++ peatland
    LOGICAL, SAVE                              :: peat_priming
    REAL(r_std), SAVE                          :: peat_priming_param
    LOGICAL, SAVE                              :: wtp_root
    INTEGER(i_std),SAVE                        :: peat_grass,peat_shrub
    LOGICAL, SAVE                              :: exudate_homogeneous
...
!!!qcj++ peatland
       peat_priming=.FALSE.
       CALL getin_p('PEAT_PRIMING',peat_priming)

       peat_priming_param=100.0
       CALL getin_p('PEAT_PRIMING_PARAM',peat_priming_param)

       wtp_root= .FALSE.
       CALL getin_p('WTP_ROOT', wtp_root)

       exudate_homogeneous= .FALSE.
       CALL getin_p('EXUDATE_HOMOGENEOUS', exudate_homogeneous)

       DO iv=1,nvm
         IF (is_peat(iv) .AND. (.NOT. is_shrubpeat(iv)) .AND. (.NOT. is_mosspeat(iv)) ) THEN
           peat_grass = iv  ! C3 peatland grass
         ENDIF
         IF (is_peat(iv) .AND. is_shrubpeat(iv)) THEN
           peat_shrub = iv  ! C3 peatland shrub
         ENDIF
       ENDDO
...
!!       END WHERE
    ...
    IF ( .NOT. no_pfrost_decomp ) THEN
            ...
            IF (wtp_root) THEN
!!!roots of peatland vegetations controlled by the water table position
              DO ip = 1, kjpindex
                 DO iv = 1, nvm
                   iz = 1
                   DO WHILE( wtpmax(ip,iv) > zi_soil(iz) .AND. iz < ndeep )
                     iz = iz + 1
                   END DO
                   wtpmax_ind(ip,iv) = iz - 1
                 ENDDO
              ENDDO
              ...
              DO iv = 1, nvm
                 IF ( is_peat(iv) ) THEN
                   DO ip = 1, kjpindex
                      IF ( ((altmax_lastyear(ip,iv) .LT. z_root_max) .OR. (wtpmax(ip,iv) .LT. z_root_max)) .and. veget_mask_2d(ip,iv) ) THEN
                         z_root(ip,iv) = MIN(altmax_lastyear(ip,iv),MAX(wtpmax(ip,iv),zi_soil(8)))  !!!MAX(wtpmax(ip),zi_soil(8)):even if the soil is flooded,
                         rootlev(ip,iv) = MIN(altmax_ind_lastyear(ip,iv),MAX(wtpmax_ind(ip,iv),8))  !!!there are roots below the water (~25cm)
                      ELSEIF (veget_mask_2d(ip,iv)) THEN
                         z_root(ip,iv) = z_root_max
                         rootlev(ip,iv) = altmax_ind_lastyear(ip,iv)
                      ENDIF
                   ENDDO

                 ELSE
                   WHERE ( altmax_lastyear(:,iv) .LT. z_root_max .and. veget_mask_2d(:,iv) )
                      z_root(:,iv) = altmax_lastyear(:,iv)
                      rootlev(:,iv) = altmax_ind_lastyear(:,iv)
                   ELSEWHERE ( veget_mask_2d(:,iv) )
                      z_root(:,iv) = z_root_max
                      rootlev(:,iv) = altmax_ind_lastyear(:,iv)
                   ENDWHERE
                 ENDIF
              ENDDO
...
!!!qcj++ peatland 
                  QCJ_tmp,wtpmax,wtp_root,root_exudate,root_exudate_z,exudate_homogeneous)
             !
!!!qcj++ peatland
             DO il = 1, ndeep
                soilcin_active(:,il,:)=(dc_litter_z(:,iactive,il,:)+root_exudate_z(:,il,:))*(zf_soil(il)-zf_soil(il-1))   !!gC/m2/time_step/model layer
             ENDDO
                 
             IF (peat_priming) THEN
                foc(:,:,:)=zero
                Kprim(:,:,:)=zero
                DO iv=1, nvm
                  DO ip = 1, kjpindex
                   IF (veget_max(ip,iv) .GT. min_stomate) THEN
                     foc(ip,:,iv)= soilcin_active(ip,:,iv)/time_step*one_day   !!gC/m2/day/model layer
                   ENDIF
                  ENDDO
                ENDDO 
                DO iv=1, nvm
                  DO ip = 1, kjpindex  
                     DO il=1, ndeep
                        IF (foc(ip,il,iv) .GT. zero) THEN
                           Kprim(ip,il,iv)=un-exp(-peat_priming_param*foc(ip,il,iv))
                        ENDIF
                     ENDDO
                  ENDDO
                ENDDO   
             ...
             CALL permafrost_decomp (kjpindex, time_step, tprof, airvol_soil, &
                  ...
                  deltaC_as,deltaC_ap,deltaC_sa,deltaC_sp,deltaC_pa,deltaC_ps, &
!!!qcj++ peatland
                  deepC_pt,deepC_peat, peat_OLT,Kprim,peat_priming)
    ...
    dc_litter_z_pftmean=zero
    deltaC1_pftmean=zero
    deltaC_as_pftmean=zero
    deltaC_ap_pftmean=zero
    deltaC_sa_pftmean=zero
    deltaC_sp_pftmean=zero
    deltaC_pa_pftmean=zero
    deltaC_ps_pftmean=zero
    do iv = 1, nvm
       do il=1,ndeep
          ...
          do ic=1,ncarb
            dc_litter_z_pftmean(:,ic,il)=dc_litter_z_pftmean(:,ic,il)+dc_litter_z(:,ic,il,iv)*veget_max(:,iv)
          end do
          deltaC_as_pftmean(:,il)=deltaC_as_pftmean(:,il)+deltaC_as(:,il,iv)*veget_max(:,iv)
          deltaC_ap_pftmean(:,il)=deltaC_ap_pftmean(:,il)+deltaC_ap(:,il,iv)*veget_max(:,iv)
          deltaC_sa_pftmean(:,il)=deltaC_sa_pftmean(:,il)+deltaC_sa(:,il,iv)*veget_max(:,iv)
          deltaC_sp_pftmean(:,il)=deltaC_sp_pftmean(:,il)+deltaC_sp(:,il,iv)*veget_max(:,iv)
          deltaC_pa_pftmean(:,il)=deltaC_pa_pftmean(:,il)+deltaC_pa(:,il,iv)*veget_max(:,iv)
          deltaC_ps_pftmean(:,il)=deltaC_ps_pftmean(:,il)+deltaC_ps(:,il,iv)*veget_max(:,iv)

          deltaC1_pftmean(:,iactive, il)=deltaC1_pftmean(:,iactive,il )+deltaC1_a(:,il,iv)*veget_max(:,iv)
          deltaC1_pftmean(:,islow  , il)=deltaC1_pftmean(:,islow  ,il )+deltaC1_s(:,il,iv)*veget_max(:,iv)
          deltaC1_pftmean(:,ipassive,il)=deltaC1_pftmean(:,ipassive,il)+deltaC1_p(:,il,iv)*veget_max(:,iv)
       end do
    end do
...
!!!qcj++ peatland
     CALL xios_orchidee_send_field ('QCJ_discret',QCJ_tmp)
     CALL xios_orchidee_send_field ('wtpmax',wtpmax)
...
! JC add for ISIMIP2b
       CALL xios_orchidee_send_field ( 'altmax_annual', altmax(:,1))
! End JC add
...
!!!qcj++ peatland
         IF (perma_peat) THEN
             ...
             CALL xios_orchidee_send_field ( 'foc', foc)
             CALL xios_orchidee_send_field ( 'Kprim', Kprim)
             CALL xios_orchidee_send_field ( 'root_exudate_z',root_exudate_z(:,:,:)/time_step)
       ...
       CALL xios_orchidee_send_field ( 'soil_in_prof_a',dc_litter_z_pftmean(:,iactive,:) )
       CALL xios_orchidee_send_field ( 'soil_in_prof_s',dc_litter_z_pftmean(:,islow,:) )
       CALL xios_orchidee_send_field ( 'soil_in_prof_p',dc_litter_z_pftmean(:,ipassive,:) )
       CALL xios_orchidee_send_field ( 'soil_resp_a',deltaC1_pftmean(:,iactive,:) )
       CALL xios_orchidee_send_field ( 'soil_resp_s',deltaC1_pftmean(:,islow,:) )
       CALL xios_orchidee_send_field ( 'soil_resp_p',deltaC1_pftmean(:,ipassive,:) )
       CALL xios_orchidee_send_field ( 'soil_a_to_s',deltaC_as_pftmean )
       CALL xios_orchidee_send_field ( 'soil_a_to_p',deltaC_ap_pftmean )
       CALL xios_orchidee_send_field ( 'soil_s_to_a',deltaC_sa_pftmean )
       CALL xios_orchidee_send_field ( 'soil_s_to_p',deltaC_sp_pftmean )
       CALL xios_orchidee_send_field ( 'soil_p_to_a',deltaC_pa_pftmean )
       CALL xios_orchidee_send_field ( 'soil_p_to_s',deltaC_ps_pftmean )
...
!!!qcj++ peatland
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL, DIMENSION(i_ind, j_ind, k_ind), INTENT(in)  :: temp_kelvin
...
!!! find corgmat value corresponding to current volumetric moisture
    IF (perma_peat) THEN
       DO ii=1,i_ind
          DO ij=1,j_ind
             DO ik=1,k_ind
                IF ( is_peat(ik) .OR. is_croppeat(ik) ) THEN
                   mc_ind = MIN(45, MAX(1, INT(moist_in(ii,ij,ik)/0.02)+1))
...
!!! peat turnover time increase with depth
    ...
    IF (perma_peat) THEN
       DO ik=1,k_ind
        IF (is_peat(ik)) THEN 
         IF ( is_shrubpeat(ik) ) THEN
           DO ij=1, j_ind
              IF (ij .LE. 12) THEN
                 peat_tau(ij,ik)= tau_peat_shrub*EXP(zi_soil(ij)/z_tau)
              ELSE
                 peat_tau(ij,ik)= tau_peat_shrub*EXP(zi_soil(12)/z_tau)
              ENDIF
           ENDDO
         ELSEIF ( is_mosspeat(ik) ) THEN
           DO ij=1, j_ind
              IF (ij .LE. 12) THEN
                 peat_tau(ij,ik)= tau_peat_moss*EXP(zi_soil(ij)/z_tau)
              ELSE
                 peat_tau(ij,ik)= tau_peat_moss*EXP(zi_soil(12)/z_tau)
              ENDIF
           ENDDO
         ELSE  ! grass peat and tree peat
           DO ij=1, j_ind
              IF (ij .LE. 12) THEN
                 peat_tau(ij,ik)= tau_peat_grass*EXP(zi_soil(ij)/z_tau)
              ELSE
                 peat_tau(ij,ik)= tau_peat_grass*EXP(zi_soil(12)/z_tau)
              ENDIF 
           ENDDO
         ENDIF 
        ENDIF  
    ...
    IF (perma_peat) THEN
      DO ik=1,k_ind
        IF ( is_peat(ik) ) THEN
           DO ij = 1, ndeep
             fbact(:,ij,ik)= peat_tau(ij,ik)/(moistfunc_result_peat(:,ij,ik)* tempfunc_result(:,ij,ik))
           ENDDO  
        ELSEIF (is_croppeat(ik)) THEN
           DO ij = 1, ndeep
              IF (ij .LE. 9) THEN
                 fbact(:,ij,ik)=tau_agrpeat/tempfunc_result(:,ij,ik) !!Peatland drainage, aerobic decompositon, here parameterized as no moisture constraint on decomposition (top 0.75m)
              ELSE
                 fbact(:,ij,ik)=tau_agrpeat/(moistfunc_result_peat(:,ij,ik)* tempfunc_result(:,ij,ik))
              ENDIF
           ENDDO
        ELSE
           DO ij = 1, ndeep
             fbact(:,ij,ik) = stomate_tau/(moistfunc_result(:,ij,ik) * tempfunc_result(:,ij,ik)) / EXP(-zi_soil(ij)/depth_modifier)
           ENDDO             
        ENDIF
      ENDDO 
    ELSE
...
!!!qcj++ peatland
    REAL(r_std), DIMENSION(kjpindex,nvm),INTENT(in)                  :: wtpmax
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm),INTENT(out)           :: QCJ_tmp
    REAL(r_std), DIMENSION(kjpindex,nvm)                             :: rprof_modified
    REAL(r_std), SAVE                                                :: altratio = 0.5
    LOGICAL, INTENT(in)                                              :: wtp_root
    LOGICAL, INTENT(in)                                              :: exudate_homogeneous
    REAL(r_std), DIMENSION(kjpindex,nvm),INTENT(in)                  :: root_exudate
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm),INTENT(out)           :: root_exudate_z
    REAL(r_std), DIMENSION(kjpindex,nvm)                             :: root_exudate_TS
    REAL(r_std), DIMENSION(kjpindex,nvm)                             :: exudate
    REAL(r_std), DIMENSION(kjpindex,nvm)                             :: exudate_correction
...
!!!qcj++ peatland
       root_exudate_TS(:,:)=root_exudate(:,:)*time_step/one_day
...
!!!qcj++ peatland
       ...
       DO ip = 1, kjpindex
          DO iv = 1, nvm
             IF (is_peat(iv) .AND. .NOT. is_shrubpeat(iv)) THEN
                 IF (altmax(ip,iv) .GT. min_stomate) THEN
                    rprof_modified(ip,iv)=1.0/(altmax(ip,iv)*altratio)
                 ELSE
                    rprof_modified(ip,iv)=rprof(ip,iv)
                 ENDIF
             ENDIF
          ENDDO
       ENDDO
       ...
       ELSE   
          IF (wtp_root) THEN
            DO ip = 1, kjpindex
               DO iv = 1, nvm
                IF (is_peat(iv) .AND. is_shrubpeat(iv)) THEN
                   z_lit(ip,iv) = MIN(rprof(ip,iv)*finerootdepthratio, altmax(ip,iv)*altrootratio, MAX(wtpmax(ip,iv)*finerootdepthratio,zi_soil(8)*finerootdepthratio))
                   intdep(ip,iv) = MIN(altmax(ip,iv), maxaltmax, MAX(wtpmax(ip,iv),zi_soil(8))) !!!MAX(wtpmax(ip),zi_soil(8)):even if the soil is flooded,
                                                                                         !!!there are roots below the water (~25cm)
                 ELSEIF (is_peat(iv) .AND. .NOT. is_shrubpeat(iv)) THEN
                   z_lit(ip,iv) = MIN(rprof_modified(ip,iv)*finerootdepthratio,altmax(ip,iv)*altrootratio,MAX(wtpmax(ip,iv)*finerootdepthratio,zi_soil(8)*finerootdepthratio))
                   intdep(ip,iv) = MIN(altmax(ip,iv)*altratio, maxaltmax, MAX(wtpmax(ip,iv),zi_soil(8)))
                 ELSE
...
!!!qcj++ peatland
       ...
       DO il = 1, ndeep
          WHERE ( zi_soil(il) .LT. intdep(:,:) .AND. veget_mask_2d(:,:) )
              QCJ_tmp(:,il,:)=1./ z_lit(:,:) / ( 1. - EXP( -intdep(:,:) / z_lit(:,:)) ) &
                            * EXP( -zi_soil(il) / z_lit(:,:) )
          ELSEWHERE
              QCJ_tmp(:,il,:)=0.
          ENDWHERE
          QCJ_tmp(:,il,:)=QCJ_tmp(:,il,:)*(zf_soil(il)-zf_soil(il-1))*carbinp_correction(:,1,:)  !!Generally carbinp_correction(:,1,:)=carbinp_correction(:,2,:)
       ENDDO
       ...
       IF (exudate_homogeneous) THEN  !! root exudates homogeneously distributed along depth (top 10 layers, total depth=1.5m)
          DO il = 1, ndeep
             WHERE ( zf_soil(il) .LE. zf_soil(10) .AND. veget_mask_2d(:,:) )
                 root_exudate_z(:,il,:) = root_exudate_TS(:,:)/10.0/(zf_soil(il)-zf_soil(il-1))
             ELSEWHERE
                 root_exudate_z(:,il,:) = zero
             ENDWHERE
          ENDDO

       ELSE  !!root exudates distribution followed root (soil C input) profile
          DO il = 1, ndeep
             WHERE ( zi_soil(il) .LT. intdep(:,:) .AND. veget_mask_2d(:,:) )
                root_exudate_z(:,il,:) = root_exudate_TS(:,:) / z_lit(:,:) / ( 1. - EXP( -intdep(:,:) / z_lit(:,:) ) ) &
                     * EXP( -zi_soil(il) / z_lit(:,:) )
             ELSEWHERE
                root_exudate_z(:,il,:) = zero
             ENDWHERE
             exudate(:,:) = exudate(:,:) + root_exudate_z(:,il,:) * (zf_soil(il)-zf_soil(il-1))
          ENDDO

          IF ( correct_carboninput_vertprof ) THEN
             WHERE ( exudate(:,:) .GT. EPSILON(0.) .AND. veget_mask_2d(:,:) )
                exudate_correction(:,:) = root_exudate_TS(:,:)/exudate(:,:)
             ELSEWHERE
                exudate_correction(:,:) = 0.
             END WHERE

             exudate(:,:)=0.
             DO il = 1, ndeep
                WHERE ( veget_mask_2d(:,:) )
                  root_exudate_z(:,il,:) = exudate_correction(:,:)*root_exudate_z(:,il,:)
                END WHERE
                exudate(:,:) = exudate(:,:) + root_exudate_z(:,il,:) * (zf_soil(il)-zf_soil(il-1)) !! check again
             END DO
          ENDIF
       ENDIF
...
!================================================================================================================================     
  ...
  SUBROUTINE permafrost_decomp (kjpindex, time_step, tprof, airvol_soil, &
       ...
       totporO2_soil, totporCH4_soil, hslong, clay, &
       no_pfrost_decomp, deepC_a, deepC_s, deepC_p, deltaCH4g, deltaCH4, deltaC1_a, deltaC1_s, deltaC1_p, deltaC2, &
       deltaC3, O2_soil, CH4_soil, fbact_out,MG_useallCpools,deltaC_as,deltaC_ap,deltaC_sa,deltaC_sp,deltaC_pa,deltaC_ps, &
!!!qcj++ peatland
       deepC_pt,deepC_peat,peat_OLT,Kprim, peat_priming)
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm), INTENT(out)  :: deltaC_as,deltaC_ap,deltaC_sa,deltaC_sp,deltaC_pa,deltaC_ps
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm), INTENT(inout)  :: O2_soil         !! oxygen (g O2/m**3 air)
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm), INTENT(inout)  :: CH4_soil        !! methane (g CH4/m**3 air)
...
!!!qcj++ peatland
    ...
    REAL(r_std), DIMENSION(kjpindex,ndeep,nvm),INTENT(in)          :: Kprim
    ...
    REAL(r_std)                                                :: Cthick
    LOGICAL,INTENT(in)                                         :: peat_priming
    !! 0.4 Local variables
...
!!! qcj++ peatland
                IF (peat_priming) THEN
                   fbact_a=fbact_a/(un+Kprim(ip,il,iv))
                ENDIF
                ...
                deltaC_as(ip,il,iv) = dC*fc(ip,iactive,islow,iv)
                deltaC_ap(ip,il,iv) = dC*fc(ip,iactive,ipassive,iv)
                !
                ! 2.2 slow       
                !
                IF (oxlim) THEN
                ...
                deltaC_sa(ip,il,iv) = dC*fc(ip,islow,iactive,iv)
                deltaC_sp(ip,il,iv) = dC*fc(ip,islow,ipassive,iv)
                !
                ! 2.3 passive
                !
                IF (oxlim) THEN
                ...
                deltaC_pa(ip,il,iv) = dC*fc(ip,ipassive,iactive,iv)
                deltaC_ps(ip,il,iv) = dC*fc(ip,ipassive,islow,iv)
                !
                !
...
!!!qcj++ peatland
    IF (perma_peat) THEN
       DO ip = 1, kjpindex
          DO il = 1, ndeep
             DO iv = 1, nvm
                IF ( is_peat(iv) .AND. veget_mask_2d(ip,iv) .AND. (.NOT. is_mineralwet(iv)) ) THEN
   ...
   !    ENDDO
       DO ip = 1, kjpindex
          DO iv=1,nvm
             IF (veget_mask_2d(ip,iv)) THEN
                peat_OLT(ip,iv) = zero
                IF ( is_peat(iv) .AND. (.NOT. is_mineralwet(iv)) ) THEN
                   il=1
