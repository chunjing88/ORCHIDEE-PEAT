...
!end gmjc
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)              :: lai             !! leaf area index OF an individual plant 
                                                                                 !! (m^2 m^{-2})           
    REAL(r_std), DIMENSION(npts,nstm), INTENT(in)             :: avail_tree      !! space availability for trees (unitless)     
    REAL(r_std), DIMENSION(npts,nstm), INTENT(in)             :: avail_grass     !! space availability for grasses (unitless)  
...
!!!qcj++ peatland
    ...
    REAL(r_std), DIMENSION(npts,nvm)                          :: fpc_peat
    REAL(r_std), DIMENSION(npts)                              :: sum_veget
    LOGICAL,DIMENSION (nstm), INTENT (in)                     :: wettile_dgvm
    REAL(r_std), DIMENSION(npts,nvm)                          :: estab_rate_max_climate_peattree
    REAL(r_std), DIMENSION(npts,nvm)                          :: estab_rate_max_peattree
...
!end gmjc
    ...
    INTEGER(i_std),DIMENSION(npts)                            :: nbtree          !! Number of PFT's which are trees
    INTEGER(i_std)                            :: i,j,k,l,m,ipts,ipart,ilit,jst       !! indices (unitless)       
!_ ================================================================================================================================
  ...
  !! 1. messages and initialization
    ...
    !IF (nbtree == 0) nbtree=1 !!!qcj++ debug
    WHERE( nbtree(:) == 0 )
       nbtree(:)=1
    ENDWHERE  
          ...
          DO k = 1, nparts ! Loop over # litter tissues
             ...
             DO l=1, npts
                IF( d_ind(l,j).GT.min_stomate .AND. &
                   ...
                   IF ((veget_cov_max_tree(l) .GT. 0.1) .AND. (veget_cov_max(l,j) .LT. veget_cov_max_tree(l)/nbtree(l)) ) THEN
             ...
             DO ipts=1, npts
                IF (d_ind(ipts,j).GT.min_stomate.AND.total_bm_c(ipts).GT.min_stomate) THEN
                   ...
                   IF ((veget_cov_max_tree(ipts) .GT. 0.1) .AND. (veget_cov_max(ipts,j) .LT. veget_cov_max_tree(ipts)/nbtree(ipts)) ) THEN
...
!!ENDDZADD
  ...
  ELSE !ok_dgvm_peat 
    ...
    IF (printlev>=3) WRITE(numout,*) 'Entering establish, ok_dgvm_peat'

    tau_eatup = one_year/2.

    IF (.NOT. lpj_gap_const_mort) THEN
      CALL ipslerr_p(3,'establish','Coherence error','when ok_dgvm_peat=T, lpj_gap_const_mort can not = F ','')
    ENDIF
    ...
    DO jst=1,nstm
       ...
       IF (wettile_dgvm(jst)) THEN   
         ...
         estab_rate_max_climate_grass(:) = zero
         estab_rate_max_climate_peattree(:,:) = zero
         estab_rate_max_grass(:) = zero
         sumfpc_wood(:) = zero
         estab_rate_max_peattree(:,:) = zero
  !! 1. messages and initialization
    ...
    ! Calculate the sum of the vegetation over the tree pft's and the number of pft's which are trees, on 'peatland'
         veget_cov_max_tree(:) = 0.0
         nbtree(:)=0
         DO j = 1, nvm
           IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_tree(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
              veget_cov_max_tree(:) = veget_cov_max_tree(:) + veget_cov_max(:,j)
              nbtree(:) = nbtree(:) + 1
           END IF
         END DO
  ...
  !! 2. recalculate fpc
       ...
       !! 2.1  peatland area in grid -> fpeat_map

         sumfpc(:) = zero

       !! 2.2 Total natural fpc on grid, in 'peatland'
       !      The overall fractional coverage of a PFT in a grid is calculated here.
       !      FPC is related to mean individual leaf area index by the Lambert-Beer law.
       !      See Eq. (1) in tex file.\n
         ...
         DO j = 2,nvm ! Loop over # PFTs
           IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
             WHERE(fpeat_map(:,jst).GT.min_stomate .AND. lai(:,j) == val_exp)
                fpc_peat(:,j) = cn_ind(:,j) * ind(:,j) / fpeat_map(:,jst)
             ELSEWHERE(fpeat_map(:,jst).GT.min_stomate .AND. lai(:,j) /= val_exp)
                fpc_peat(:,j) = cn_ind(:,j) * ind(:,j) / fpeat_map(:,jst) &
                 * ( un - exp( - lm_lastyearmax(:,j) * sla_calc(:,j) * ext_coeff(j) ) )
             ENDWHERE

             WHERE ( PFTpresent(:,j) )
                sumfpc(:) = sumfpc(:) + fpc_peat(:,j)
             ENDWHERE
           ELSE
             fpc_peat(:,j) = zero
           ENDIF
       ...
       !! 2.3 Total woody fpc on grid and number of regenerative tree pfts, in 'peatland'
       !      Total woody FPC increases by adding new FPC.
       !      Under the condition that temperature in last winter is higher than a threshold,
       !      woody plants is exposed in higher competitive environment.
         sumfpc_wood(:) = zero
         ...
         DO j = 2,nvm ! Loop over # PFTs
           IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_tree(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
             WHERE ( PFTpresent(:,j) )
                sumfpc_wood(:) = sumfpc_wood(:) + fpc_peat(:,j)
             ENDWHERE
           ENDIF
         ENDDO ! Loop over # PFTs
       ...
       !     Grass increment equals 'everywhere'\n
         ...
         spacefight_grass(:) = zero
         DO j = 2,nvm ! Loop over # PFTs
           IF ( (.NOT. is_tree(j)) .AND. natural(j) .AND. (.NOT. pasture(j)) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
          ! Count a PFT fully only if it is present on a grid.
              WHERE ( PFTpresent(:,j) )
                 spacefight_grass(:) = spacefight_grass(:) + everywhere(:,j)
              ENDWHERE
           ...
           IF ( natural(j) .AND. (.NOT. pasture(j)) .AND. is_peat(j) ) THEN
              IF (.NOT. is_tree(j)) THEN
                ...
                ELSEWHERE
                  estab_rate_max_climate_grass(:) = zero
                ENDWHERE    
              ELSEIF ( leaf_tab(j)==1 .AND. pheno_type(j)==2 ) THEN

                WHERE ( ( precip_annual(:) .GE. precip_crit ) .AND. ( gdd0(:) .GE. gdd_crit_estab ) )
                  estab_rate_max_climate_peattree(:,j) = estab_max_PeatBS
                ELSEWHERE
                  estab_rate_max_climate_peattree(:,j) = zero
                ENDWHERE
              ELSE
                WHERE ( ( precip_annual(:) .GE. precip_crit ) .AND. ( gdd0(:) .GE. gdd_crit_estab ) )
                  estab_rate_max_climate_peattree(:,j) = estab_max_PeatTree
                ELSEWHERE
                  estab_rate_max_climate_peattree(:,j) = zero 
                ENDWHERE 
              ENDIF
       ...
       !! 2.6 Reduce maximum tree establishment rate if many trees are present.
         ...
         DO j = 2,nvm
           IF ( natural(j) .AND. (.NOT. pasture(j)) .AND. is_peat(j) .AND. (is_tree(j))) THEN  
             estab_rate_max_peattree(:,j) = estab_rate_max_climate_peattree(:,j) * factor(:)
           ENDIF  
         ENDDO  
       !! 2.7 Modulate grass establishment rate.
       !      If canopy is not closed (fpc < fpc_crit-0.05), normal
       !      establishment.
       !      If canopy is closed, establishment is reduced by a factor 4.
         estab_rate_max_grass(:) = MAX(MIN(estab_rate_max_climate_grass(:), max_tree_coverage - sumfpc(:)),zero)
       ...
       !      is possible for the more productive PFT
         factor(:) = min_stomate
         DO j = 2,nvm ! Loop over # PFTs
          IF ( natural(j) .AND. (.NOT.is_tree(j)) .AND. (.NOT. pasture(j)) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN
            factor(:) = factor(:) + npp_longterm(:,j) * &
            lm_lastyearmax(:,j) * sla_calc(:,j)
          ENDIF 
         ENDDO ! Loop over # PFTs
    ...
    !! 2.9 Establish natural PFTs
         ...
         DO i = 1, npts    
            DO j = 2,nvm ! Loop over # PFTs
               IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND.(pref_soil_veg(j)==jst) ) THEN
                  sum_veget(i)=sum_veget(i)+veget_cov_max(i,j)
               ENDIF
            ENDDO
         ENDDO
         ...
         DO j = 2,nvm ! Loop over # PFTs
           ...
           IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN ! only for natural PFTs, in peatland
             !! 2.9.1 PFT expansion across the grid box. Not to be confused with areal coverage.
             IF ( treat_expansion ) THEN
                ! only treat plants that are regenerative and present and still can expand
                DO i = 1, npts ! Loop over # pixels - domain size
             ...
             IF ( is_tree(j) ) THEN
                ...
                !WHERE ( PFTpresent(:,j) .AND. ( regenerate(:,j) .GT. regenerate_crit ) .AND. ( sum_veget(:) .LT. fpeat_map(:)*fpc_crit ) )
                WHERE ( PFTpresent(:,j) .AND. ( regenerate(:,j) .GT. regenerate_crit ) ) 
                   d_ind(:,j) = estab_rate_max_peattree(:,j)*everywhere(:,j) * &
                        avail_tree(:,jst) * dt/one_year
                ENDWHERE
             ...
             ELSE
                ...
                !WHERE ( PFTpresent(:,j) .AND. ( regenerate(:,j) .GT. regenerate_crit )  &
                !     .AND. factor(:).GT.min_stomate .AND. spacefight_grass(:).GT. min_stomate .AND. ( sum_veget(:) .LT. fpeat_map(:)*fpc_crit ) )
                WHERE ( PFTpresent(:,j) .AND. ( regenerate(:,j) .GT. regenerate_crit )   &
                      .AND. factor(:) .GT. min_stomate .AND. spacefight_grass(:) .GT. min_stomate )

                   d_ind(:,j) = estab_rate_max_grass(:)*everywhere(:,j)/spacefight_grass(:) * &
                                MAX(min_stomate,npp_longterm(:,j)*lm_lastyearmax(:,j) * &
                                sla_calc(:,j)/factor(:)) * fpeat_map(:,jst) * dt/one_year
                ENDWHERE

             ENDIF  ! tree/grass

           ENDIF ! if natural .AND. is_peat
  ...
  !! 4. Biomass calculation
         DO j = 2,nvm ! Loop over # PFTs
          ...
          IF ( natural(j) .AND. .NOT. pasture(j) .AND. is_peat(j) .AND. (pref_soil_veg(j)==jst) ) THEN ! only for natural PFTs
             ...
             IF(is_tree(j))THEN
                ...
                DO i=1,npts ! Loop over # pixels - domain size
                   ...
                   IF((d_ind(i,j)+ind(i,j)).GT.min_stomate) THEN
                      ...
                      ENDIF
                      ! new diameter of PFT
                      IF (.NOT. pipe_shrub) THEN
                        dia(i) = (woodmass_ind(i,j)/(pipe_density*pi/4.*pipe_tune2)) &
                           &                **(1./(2.+pipe_tune3))
                        vn(i) = (ind(i,j) + d_ind(i,j))*pipe_tune1*MIN(dia(i),maxdia(j))**pipe_tune_exp_coeff
                      ELSE
                        IF (.NOT. is_shrubpeat(j)) THEN
                           dia(i) = (woodmass_ind(i,j)/(pipe_density*pi/4.*pipe_tune2)) &
                              &                **(1./(2.+pipe_tune3))
                           vn(i) = (ind(i,j) + d_ind(i,j))*pipe_tune1*MIN(dia(i),maxdia(j))**pipe_tune_exp_coeff
                        ELSE
                           dia(i) = (woodmass_ind(i,j)/(pipe_density*pi/4.*8.0))&
                              &                **(1./(2.+0.55))
                           vn(i) = (ind(i,j) + d_ind(i,j))*216.9*MIN(dia(i),maxdia(j))**pipe_tune_exp_coeff
                        ENDIF
                      ENDIF
                   ENDIF
...
!corresponding biomass has been lost in lpj_gap).
               ...
               DO l=1, npts
                 IF( d_ind(l,j).GT.min_stomate .AND. &
                   ...
                   ELSE
                      ...
                      bm_non(l) = MIN( bm_to_litter(l,j,k,icarbon), &
                        (ind(l,j)+d_ind(l,j))*mortality(l,j) * bm_sapl(j,k,icarbon)/veget_cov_max(l,j) )
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check nparts k,',k
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check ind,',ind(l,j),'d_ind',d_ind(l,j)
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check mortality',mortality(l,j)
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check bm_sapl',bm_sapl(j,k,icarbon)
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check veget_cov_max,',veget_cov_max(l,j)
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check bm_new,',bm_new(l),'bm_non',bm_non(l),'bm_to_litter',bm_to_litter(l,j,k,icarbon)
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check co2_to_bm, lpj_establish 3,',j,co2_to_bm(l,j)
                      co2_to_bm(l,j)=co2_to_bm(l,j) + bm_new(l)/dt - bm_non(l)/dt
                      !IF (j==18 .OR. j==19)  WRITE (numout,*) 'QCJ check co2_to_bm, lpj_establish 4,',j,co2_to_bm(l,j)
  ...
  !! 5. history
    ...
    IF (printlev>=4) WRITE(numout,*) 'Leaving establish, ok_dgvm_peat'
