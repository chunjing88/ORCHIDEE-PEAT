...
!!
!! SVN		:
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-MICT/ORCHIDEE/src_stomate/stomate_vmax.f90 $ 
!! $Date: 2018-03-09 16:35:40 +0100 (Fri, 09 Mar 2018) $
!! $Revision: 5082 $
!! \n
!_ =================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE vmax (npts, dt, &
       ...
       vcmax, &!)
!gmjc
       N_limfert,humrel_month) !!!qcj++ peatland, following Druel et al. 2017 GMD paper,add humrel_month for dessication impact
!end gmjc
    !
...
!! (see section [118] in Krinner et al. (2005)).
    ...
    DO j = 2,nvm
       ...
       ELSE 
!!!qcj++ peatland
        IF (ok_dgvm_peat .AND. pheno_type(j)==1 .AND. leaf_tab(j)==2 .AND. is_peat(j)) THEN
            vcmax(:,j) = Vcmax25(j)
        ELSE            
          ! for deciduous tree
          DO m = 1, nleafages       ! Loop over age classes
...
!!!qcj++ peatland, following Druel et al. 2017 GMD paper, Eq.6
          IF ( is_mosspeat(j) ) THEN
             WHERE ( humrel_month(:,j) .LT. humrel_mmin )
                 vcmax(:,j) = vcmax(:,j) * ( vcmax_offset + ((1.0 - vcmax_offset) / humrel_mmin ) * humrel_month(:,j))
             ENDWHERE
          ENDIF
        ENDIF 
