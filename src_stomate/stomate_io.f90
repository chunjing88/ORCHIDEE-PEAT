...
!!!qcj++ peatland
    ! carbon of acrotelm (gC/m**2)
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out):: carbon_acro
    ! carbon of catotelm (gC/m**2)
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out):: carbon_cato
    ! height of acrotelm 
    REAL(r_std),DIMENSION(npts),INTENT(out):: height_acro
    !wetland fraction
    REAL(r_std),DIMENSION(npts),INTENT(out):: fwet_daily
    REAL(r_std),DIMENSION(npts),INTENT(out):: liqwt_daily
    REAL(r_std),DIMENSION(npts),INTENT(out):: fwet_month
    REAL(r_std),DIMENSION(npts),INTENT(out):: liqwt_month
    REAL(r_std),DIMENSION(npts),INTENT(out):: liqwt_max
    INTEGER(i_std),INTENT(in) :: months_num
    REAL(r_std),DIMENSION(npts,months_num),INTENT(out):: fwet_series
    REAL(r_std),DIMENSION(npts),INTENT(out):: fpeat
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out):: wtpmax 
    REAL(r_std), DIMENSION(npts,ndeep,nvm),INTENT(out) ::deepC_peat
    ...
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out)     :: npp0_cumul
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out)      :: wtp_month
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out)      :: wtpmax_month
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out)      :: wtp_year
    !-
...
!!!qcj++ peatland
    ...
    var_name = 'fwet_daily'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., fwet_daily, 'gather', nbp_glo,index_g)
    IF (ALL(fwet_daily(:) == val_exp)) THEN
        fwet_daily(:) = zero
    ENDIF

    liqwt_daily(:) = val_exp
    var_name = 'liqwt_daily'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., liqwt_daily, 'gather', nbp_glo,index_g)
    IF (ALL(liqwt_daily(:) == val_exp)) THEN
        liqwt_daily(:) = zero
    ENDIF
    ...
    var_name = 'fwet_month'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., fwet_month, 'gather', nbp_glo,index_g)
    IF (ALL(fwet_month(:) == val_exp)) THEN
        fwet_month(:) = zero
    ENDIF

    liqwt_month(:) = val_exp
    var_name = 'liqwt_month'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., liqwt_month, 'gather', nbp_glo,index_g)
    IF (ALL(liqwt_month(:) == val_exp)) THEN
        liqwt_month(:) = zero
    ENDIF
    ...
    var_name = 'liqwt_max'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., liqwt_max, 'gather', nbp_glo,index_g)
    IF (ALL(liqwt_max(:) == val_exp)) THEN
        liqwt_max(:) = zero
    ENDIF

    fwet_series(:,:) = val_exp
    var_name = 'fwet_series'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, months_num, 1, itime, &
    &               .TRUE., fwet_series, 'gather', nbp_glo,index_g)
    IF (ALL(fwet_series(:,:) == val_exp)) THEN
        fwet_series(:,:) = zero
    ENDIF
    ...
    var_name = 'fpeat'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, 1, 1, itime, &
    &               .TRUE., fpeat, 'gather', nbp_glo,index_g)
    IF (ALL(fpeat(:) == val_exp)) THEN
        fpeat(:) = zero
    ENDIF
    ...
    var_name = 'wtpmax'
    CALL restget_p (rest_id_stomate,var_name, nbp_glo, nvm, 1, itime, &
    &               .TRUE., wtpmax, 'gather', nbp_glo,index_g)
    IF (ALL(wtpmax(:,:) == val_exp)) THEN
        wtpmax(:,:) = zero
    ENDIF
    ...
    var_name = 'deepC_peat'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, ndeep, nvm, itime, &
         &               .TRUE., deepC_peat, 'gather', nbp_glo, index_g)
    IF (ALL(deepC_peat == val_exp)) deepC_peat(:,:,:) = zero 

    carbon_save(:,:,:) = val_exp
    var_name = 'carbon_save'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, ncarb , nvm, itime, &
         &                   .TRUE., carbon_save, 'gather', nbp_glo, index_g)
    IF (ALL(carbon_save == val_exp)) carbon_save(:,:,:) = zero
 

    deepC_a_save(:,:) = val_exp
    var_name = 'deepC_a_save'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               .TRUE., deepC_a_save, 'gather', nbp_glo, index_g)
    IF (ALL(deepC_a_save == val_exp)) deepC_a_save(:,:) = zero 

    deepC_s_save(:,:) = val_exp
    var_name = 'deepC_s_save'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               .TRUE., deepC_s_save, 'gather', nbp_glo, index_g)
    IF (ALL(deepC_s_save == val_exp)) deepC_s_save(:,:) = zero

    deepC_p_save(:,:) = val_exp
    var_name = 'deepC_p_save'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               .TRUE., deepC_p_save, 'gather', nbp_glo, index_g)
    IF (ALL(deepC_p_save == val_exp)) deepC_p_save(:,:) = zero

    delta_fsave(:) = val_exp
    var_name = 'delta_fsave'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &               .TRUE., delta_fsave, 'gather', nbp_glo, index_g)
    IF (ALL(delta_fsave == val_exp)) delta_fsave(:) = zero
   
    npp0_cumul(:,:) = val_exp
    var_name = 'npp0_cumul'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nvm  , 1, itime, &
         &              .TRUE., npp0_cumul, 'gather', nbp_glo, index_g)
    IF (ALL(npp0_cumul(:,:) == val_exp)) npp0_cumul(:,:) = zero

    wtp_month(:,:) = val_exp
    var_name = 'wtp_month'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nvm   , 1, itime, &
         &              .TRUE., wtp_month, 'gather', nbp_glo, index_g)
    IF (ALL(wtp_month(:,:) == val_exp)) wtp_month(:,:) = zero

    wtpmax_month(:,:) = val_exp
    var_name = 'wtpmax_month'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nvm   , 1, itime, &
         &              .TRUE., wtpmax_month, 'gather', nbp_glo, index_g)
    IF (ALL(wtpmax_month(:,:) == val_exp)) wtpmax_month(:,:) = zero

    wtp_year(:,:) = val_exp
    var_name = 'wtp_year'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nvm   , 1, itime, &
         &              .TRUE., wtp_year, 'gather', nbp_glo, index_g)
    IF (ALL(wtp_year(:,:) == val_exp)) wtp_year(:,:) = zero
...
!!!qcj++ peatland
    ...
    !wetland fraction
    REAL(r_std),DIMENSION(npts),INTENT(in) :: fwet_daily
    REAL(r_std),DIMENSION(npts),INTENT(in) :: liqwt_daily
    REAL(r_std),DIMENSION(npts),INTENT(in) :: fwet_month
    REAL(r_std),DIMENSION(npts),INTENT(in) :: liqwt_month
    REAL(r_std),DIMENSION(npts),INTENT(in) :: liqwt_max
    REAL(r_std),DIMENSION(npts),INTENT(in) :: fpeat
    REAL(r_std),DIMENSION(npts,nvm),INTENT(in) :: wtpmax
    INTEGER(i_std),INTENT(in) ::              months_num
    REAL(r_std),DIMENSION(npts,months_num),INTENT(in) :: fwet_series
    REAL(r_std), DIMENSION(npts,ndeep,nvm),INTENT(in) :: deepC_peat
    REAL(r_std), DIMENSION(npts,ncarb,nvm), INTENT(in)     :: carbon_save
    REAL(r_std), DIMENSION(npts,ndeep), INTENT(in)         ::deepC_a_save
    REAL(r_std), DIMENSION(npts,ndeep), INTENT(in)         :: deepC_s_save
    REAL(r_std), DIMENSION(npts,ndeep), INTENT(in)         :: deepC_p_save
    REAL(r_std), DIMENSION(npts), INTENT(inout)               :: delta_fsave
    ...
    REAL(r_std),DIMENSION(npts,nvm),INTENT(in) :: wtp_year
!gmjc
...
!!!qcj++ peatland
    ...
    var_name = 'fwet_daily'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              fwet_daily, 'scatter', nbp_glo, index_g)

    var_name = 'liqwt_daily'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              liqwt_daily, 'scatter', nbp_glo, index_g)

    var_name = 'fwet_month'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              fwet_month, 'scatter', nbp_glo, index_g)

    var_name = 'liqwt_month'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              liqwt_month, 'scatter', nbp_glo, index_g)

    var_name = 'liqwt_max'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              liqwt_max, 'scatter', nbp_glo, index_g)

    var_name = 'fwet_series'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, months_num, 1, itime, &
         &              fwet_series, 'scatter', nbp_glo, index_g)

    var_name = 'fpeat'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &              fpeat, 'scatter', nbp_glo, index_g)

    var_name = 'wtpmax'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
         &              wtpmax, 'scatter', nbp_glo, index_g)

    var_name = 'deepC_peat'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, ndeep, nvm, itime, &
         &               deepC_peat, 'scatter', nbp_glo, index_g)

    var_name = 'carbon_save'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, ncarb, nvm, itime, &
         &                   carbon_save, 'scatter', nbp_glo, index_g)

    var_name = 'deepC_a_save'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               deepC_a_save, 'scatter', nbp_glo, index_g)

    var_name = 'deepC_s_save'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               deepC_s_save, 'scatter', nbp_glo, index_g)

    var_name = 'deepC_p_save'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, ndeep, 1, itime, &
         &               deepC_p_save, 'scatter', nbp_glo, index_g)

    var_name = 'delta_fsave'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, 1, 1, itime, &
         &               delta_fsave, 'scatter', nbp_glo, index_g)

    var_name = 'npp0_cumul'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
         &                npp0_cumul, 'scatter', nbp_glo, index_g)

    var_name = 'wtp_month'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
                         wtp_month, 'scatter', nbp_glo, index_g)

    var_name = 'wtpmax_month'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
                         wtpmax_month, 'scatter', nbp_glo, index_g)

    var_name = 'wtp_year'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
                         wtp_year, 'scatter', nbp_glo, index_g)
