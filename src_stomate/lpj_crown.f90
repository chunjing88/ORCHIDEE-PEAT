...
!!!qcj++ peatland
  ...
  ELSE
    IF (ok_dgvm) THEN
      CALL ipslerr_p(3,'crown','Coherence error','ok_dgvm_peat and ok_dgvm can not be true at the same time.','')
    ENDIF
    ...
    DO j = 2,nvm ! loop over PFTs
       IF (is_peat(j)) THEN
          cn_ind(:,j) = zero
          height_presc_12(j) = height_presc(j)
       ENDIF
    ENDDO
          ...
          IF (natural(j)) THEN
             ...
             ELSE
               IF (.NOT. is_shrubpeat(j)) THEN
                  ...
                  cn_ind(:,j) = pipe_tune1*MIN(dia(:),maxdia(j))**pipe_tune_exp_coeff       
               ELSE
                  dia(:) = (woodmass_ind(:,j)/(pipe_density*pi/4.*8.0)) &
                  &       **(1./(2.+0.55))
                  height(:,j) = 8.0*(dia(:)**0.55)
                  cn_ind(:,j) = 216.9*MIN(dia(:),maxdia(j))**pipe_tune_exp_coeff
