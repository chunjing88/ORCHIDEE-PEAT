...
!!!qcj++ peatland
  LOGICAL, SAVE :: ok_peat = .FALSE.                     !!Flag to activite peatland carbon accumumodule, from Kleinen et al,2012
  LOGICAL, SAVE :: peat_occur = .FALSE.
  LOGICAL, SAVE :: dyn_peat = .FALSE.
  LOGICAL, SAVE :: dynpeat_PWT = .FALSE.
  LOGICAL, SAVE :: dynpeat_PC = .FALSE.
  LOGICAL, SAVE :: perma_peat = .FALSE.
  LOGICAL, SAVE :: update_peatfrac = .FALSE.
  LOGICAL, SAVE :: done_update_peatfrac = .FALSE.
  LOGICAL, SAVE :: agri_peat = .FALSE.
  LOGICAL, SAVE :: agri_peat_prop = .FALSE.
  LOGICAL, SAVE :: agri_peat_MAXcrop = .FALSE.
  LOGICAL, SAVE :: agri_peat_MINcrop = .FALSE.
  LOGICAL, SAVE :: ok_dgvm_peat = .FALSE.
  LOGICAL, SAVE :: pipe_shrub = .FALSE.
  LOGICAL, SAVE :: peat_hydro = .FALSE.
  !
...
!!!qcj++ peatland
  LOGICAL, SAVE :: wtp_constraint = .FALSE.
!$OMP THREADPRIVATE(wtp_constraint)
  REAL(r_std), SAVE :: factor_wtp = 4.0
!$OMP THREADPRIVATE(factor_wtp)
...
!stomate_soilcarbon.f90
!
  REAL(r_std), SAVE :: p_A=3.5E+4     !!acrotelm density (g/m3)
  REAL(r_std), SAVE :: p_C=9.1E+4     !!catotelm density (g/m3)
  REAL(r_std), SAVE :: cf_A=0.50      !!carbon fraction in acrotelm peat
  REAL(r_std), SAVE :: cf_C=0.52      !!carbon fraction in catotelm peat
  REAL(r_std), SAVE :: v_ratio=0.35         !!ratio anaerobic-aerobic CO2
  REAL(r_std), SAVE :: KA_ini=0.067   !!acrotelm decomposition rate (an-1)
  REAL(r_std), SAVE :: KP_ini=1.91E-2 !!catotelm formation rate (an-1)
  REAL(r_std), SAVE :: KC_ini=3.35E-5 !!catotelm decomposition rate (an-1)
  REAL(r_std), SAVE :: tau_fwet_month = 15. !!! Time scale for memory of the fwet index (days).
  REAL(r_std), SAVE :: tau_liqwt_month = 20.
  REAL(r_std), SAVE :: frac1=0.95
  REAL(r_std), SAVE :: frac2=0.05
  REAL(r_std), SAVE :: numlayers=11
  REAL(r_std), SAVE :: liqlayers=10
  REAL(r_std), PARAMETER :: ini_peat = 1.E-6_r_std
  REAL(r_std), SAVE :: tau_wtp_year = 300.        !! (days)       
  REAL(r_std), SAVE :: estab_max_PeatTree = 0.12 
  REAL(r_std), SAVE :: estab_max_PeatBS = 0.12

!$OMP THREADPRIVATE(tau_wtp_year)

!!! qcj++ peatland, following Druel et al. 2017 GMD paper
  REAL(r_std), SAVE :: rho_moss = 0.5E4  !! density of moss (gC/m**3)
  REAL(r_std), SAVE :: init_sapl_mass_leaf_novasc = 0.01
!!! qcj++ peatland impact of drought on the desiccation of moss, following Druel et al. 2017 GMD paper, Eq.6
  REAL(r_std), SAVE :: vcmax_offset = 0.55
  REAL(r_std), SAVE :: humrel_mmin = 0.8
...
!$OMP THREADPRIVATE(leaf_age_crit_coeff)
  ...
  REAL(r_std), SAVE, DIMENSION(4) :: npp0_c = (/20., 60., 130., 0.005/) !!!qcj++ peatland, coefficients for Eq.3 in Druel et al. 2017 GMD
  REAL(r_std), SAVE :: llai_coef = 0.014  !!!qcj++ peatland, coefficients for Eq.4 in Druel et al. 2017 GMD
