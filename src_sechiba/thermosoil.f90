...
!! $Date: 2018-06-11 10:43:59 +0200 (Mon, 11 Jun 2018) $
!! $Revision: 5283 $
!! \n
!_ ================================================================================================================================
...
!! Soil texture choose
    ...
    IF (ALL(refSOC(:,:) == val_exp)) THEN
       IF (use_refSOC) THEN
         CALL read_refSOCfile(kjpindex,lalo,neighbours, resolution, contfrac)
       ENDIF
    ENDIF
...
!_ ================================================================================================================================
  ...
  !! 5. Call to thermosoil_energy_diag for calculation of diagnostic variables
    CALL thermosoil_energy_diag (kjpindex, temp_sol_new,  soilcap,  veget_max_bg)
    ptn_pftmean(:,:) = zero
    DO m=1,nvm
       DO jg = 1, ngrnd
          ptn_pftmean(:,jg) = ptn_pftmean(:,jg) + ptn(:,jg,m) * veget_max_bg(:,m)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE thermosoil_coef (kjpindex, temp_sol_new, temp_sol_new_pft, snow,              &
    ...
    REAL(r_std),DIMENSION (kjpindex)                       :: tmp1
    REAL(r_std),DIMENSION (kjpindex)                       :: tmp2
    REAL(r_std),DIMENSION (kjpindex)                       :: tmp3
...
!    ENDIF
    ...
    !! 2.2 Calculate coefficients cgrnd and dgrnd for soil
       ...
       DO jg = ngrnd-1,2,-1
          tmp1(:)=zdz1(:,jg,jv) * (un - dgrnd(:,jg,jv))
          tmp2(:)=zdz2(:,jg,jv) + zdz1(:,jg-1,jv) +tmp1
          tmp3(:)=un/tmp2 
       ...
       !   IF (jv==16 .AND. jg==7) THEN
       !       WRITE (numout,*) 'QCJ check tmp1,',tmp1(:)
       !       WRITE (numout,*) 'QCJ check tmp2,',tmp2(:)
       !       WRITE (numout,*) 'QCJ check tmp3,',tmp3(:)
       !       WRITE (numout,*) 'QCJ check dgrnd jg-1,',jg,dgrnd(:,jg-1,jv)
       !       WRITE (numout,*) 'QCJ check z1 jg,',jg,z1(:)
       !   ENDIF
...
!_ ================================================================================================================================
  ...
  !! 2. Computation of the apparent ground heat flux 
    ...
    DO ji = 1, kjpindex
     ...
     !  WRITE (numout,*) 'QCJ check soilflx in thermosoil',soilflx(ji)
     !  WRITE (numout,*) 'QCJ check snowflx',snowflx(ji)
     !  WRITE (numout,*) 'QCJ check soilflx_nosnow',soilflx_nosnow(ji)
    ENDDO
...
!_ ================================================================================================================================
  ...
  !! 0. Variables and parameter declaration
    ...
    !! 0.2 Output variables
    REAL(r_std), DIMENSION(:,:), INTENT(out):: mc_layt                   !! Volumetric soil moisture (liquid+ice) (m3/m3) on the thermodynamical levels at interface
    REAL(r_std), DIMENSION(:,:), INTENT(out):: mcl_layt                  !! Volumetric soil moisture (liquid) (m3/m3) on the thermodynamical levels at interface
    REAL(r_std), DIMENSION(:,:), INTENT(out):: tmc_layt                  !! Total soil moisture content for each layer (liquid+ice) (mm) on the thermodynamical levels
    REAL(r_std), DIMENSION(:,:,:), INTENT(out):: mc_layt_pft             !! Volumetric soil moisture (liquid+ice) (m3/m3) on the thermodynamical levels at interface
    REAL(r_std), DIMENSION(:,:,:), INTENT(out):: mcl_layt_pft            !! Volumetric soil moisture (liquid) (m3/m3) on the thermodynamical levels at interface
    REAL(r_std), DIMENSION(:,:,:), INTENT(out):: tmc_layt_pft            !! Total soil moisture content for each layer (liquid+ice) (mm) on the thermodynamical levels
    REAL(r_std), DIMENSION(:,:,:), INTENT(out):: shum_ngrnd_perma        !! Saturation degree on the thermal axes (0-1, dimensionless) 
...
#endif
       CALL thermosoil_freeze_thermix(kjpindex, ngrnd, nvm, ptn, shum_ngrnd_permalong, &
...
!! \n 
!_ ================================================================================================================================
   SUBROUTINE thermosoil_freeze_thermix(kjpindex, ngrnd, nvm, ptn, shum_ngrnd_permalong, & 
                    mc_layt, mc_layt_pft, tmc_layt_pft, so_capa_dry_net, dlt, &
                    profil_froz, pcapa, pcappa_supp) ! out
           ...
           DO ji = 1,kjpindex
              ...
              IF (ptn(ji,jg,jv) .LT. ZeroCelsius-fr_dT/2.) THEN
                 ...
                 !! this is from Koven's version: pcapa(ji,jg,jv) = so_capa_dry_net + shum_ngrnd_permalong(ji,jg,jv)*poros_net(ji,jg,jv)*capa_ice*rho_ice
                  ...
                  ELSE
                      pcapa(ji,jg,jv) = so_capa_dry_net(ji,jg,jv) + so_capa_ice * mc_layt(ji,jg)
                  ENDIF
...
!_ ================================================================================================================================
  ...
  SUBROUTINE read_refSOCfile(nbpt, lalo, neighbours, resolution, contfrac)
...
!================================================================================================================================
  ...
  IF (lml .NE. ngrnd) THEN
    ...
    CALL ipslerr_p(3, 'read_refSOCfile',    &
  ...
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Problem in allocation of variable lon_lu','','')

  ALLOCATE(lat_lu(jml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Problem in allocation of variable lat_lu','','')

  ALLOCATE(mask_lu(iml,jml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for mask_lu','','')

  ALLOCATE(refSOC_file(iml,jml,lml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for refSOC_file','','')
  ...
  IF (ANY(refSOC_file .NE. refSOC_file)) THEN
     CALL ipslerr_p(3,'read_refSOCfile','Filename:'//filename,          &
                      'variable: soil_organic_carbon',                  &
                      'Nan values not allowed. Check the input data')
  ENDIF

  ALLOCATE(lon_rel(iml,jml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for lon_rel','','')

  ALLOCATE(lat_rel(iml,jml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for lat_rel','','')

  ALLOCATE(mask(iml,jml), STAT=ALLOC_ERR)
  IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Problem in allocation of variable mask','','')
  ...
  DO WHILE ( .NOT. ok_interpol )
     ...
     IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for sub_area','','')
     sub_area(:,:)=zero

     ALLOCATE(sub_index(nbpt,nbvmax,2), STAT=ALLOC_ERR)
     IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'read_refSOCfile','Pb in allocation for sub_index','','')
     sub_index(:,:,:)=0
