...
!$OMP THREADPRIVATE(soilmoist)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:,:) :: soilmoist_s     !! Total soil moisture content for each layer in hydrol(liquid + ice) (mm)
...
!!!qcj++ peatland
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: wtp
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)      :: fwet_new
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)      :: liqwt_ratio
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)      :: fpeat
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)      :: mc_peat_above
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:,:)    :: shumdiag_peat
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: wtpmax
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: wtp_year
!  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: veget_max_adjusted
...
!_ ================================================================================================================================
    ...
    CALL slowproc_initialize (kjit,     kjpij,          kjpindex,       date0,          &      
                        ...
                        f_rot_sech, altmax, fpeat, wtpmax, wtp_year)
...
!! 0.4 local variables
!!!qcj++ peatland
    LOGICAL,DIMENSION (nstm)                                 :: wettile_dgvm
    CHARACTER(LEN=25), DIMENSION(nstm)                       :: tile_name_dgvm
...
!!! qcj++
    REAL(r_std), DIMENSION (kjpindex)                        :: peat_PET
    REAL(r_std),DIMENSION (kjpindex,nvm)                     :: veget_cov_max_new_peatdgvm
!_ ================================================================================================================================
    ...
    CALL diffuco_main (kjit, kjpindex, index, indexveg, indexlai, indexlai0, u, v, &
         ...
         & hist_id, hist2_id, wtp_year) !qcj++ peatland
...
!            & humrel, vegstress, drysoil_frac, evapot, evapot_corr, evap_bare_lim, evap_bare_lim_pft, flood_frac, flood_res, &
            ...
            & drunoff_tot, fwet_out, lambda_snow, cgrnd_snow, dgrnd_snow, temp_sol_add, &
!gmjc
...
!!! qcj++ peatland
    IF (dynpeat_PWT) THEN
       DO ji = 1, kjpindex
          peat_PET(ji)= 0 
          peat_PET(ji) = peat_PET(ji)+evapot(ji)*veget_max(ji,1)
          DO jv = 2,nvm
          !   IF (is_peat(jv)) THEN
                IF (veget_max(ji,jv) .GT. 0 ) THEN
                    tempfrac = veget(ji,jv)/veget_max(ji,jv)
                ...
                ELSE
                    tempfrac = 0
                ENDIF
                peat_PET(ji)=peat_PET(ji)+ (transpot(ji,jv)*tempfrac + evapot(ji)*(1-tempfrac))*veget_max(ji,jv)
          !   ENDIF
          ENDDO
       ENDDO
    ENDIF
...
!!!qcj++ peatland
         ...
         wtp_year,wettile_dgvm,tile_name_dgvm )!,veget_max_adjusted)
!,vevapp)
...
! JC add for ISIMIP2b output
    histvar(:)=zero
    DO ji = 1, kjpindex
       IF (SUM(veget_max(ji,:)) > min_sechiba) THEN
         DO jv=2,nvm
            histvar(ji) = histvar(ji) + veget_max(ji,jv)*gsmean(ji,jv)/SUM(veget_max(ji,:))
         END DO
       END IF
    END DO
    CALL xios_orchidee_send_field("gsgrid",histvar)
!End JC add
...
!!!qcj++
   IF (done_stomate_lcchange .AND. .NOT. dyn_peat) THEN
...
!       irrigation(:) = zero
    ...
    CALL slowproc_finalize (kjit,       kjpindex,  rest_id,  index,      &
                           ...
                           assim_param, frac_age,altmax,fpeat,wtpmax, wtp_year)  !qcj++ peatland
...
!pss:-
!!!qcj++ peatland
    ALLOCATE (wtp(kjpindex,nvm),stat=ier)
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in wtp allocation. We stop. we need kjpindex*nstm words = ',kjpindex,nvm
       STOP 'sechiba_init'
    END IF
    wtp(:,:) = undef_sechiba
 
    ALLOCATE (wtp_year(kjpindex,nvm),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for wtp_year','','')
  
    ALLOCATE (fwet_new(kjpindex),stat=ier)
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in fwet_new allocation. We stop. we need kjpindex words = ',kjpindex
       STOP 'sechiba_init'
    END IF
    fwet_new(:) = undef_sechiba

    ALLOCATE (liqwt_ratio(kjpindex),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for liqwt_ratio','','')

    ALLOCATE (fpeat(kjpindex),stat=ier)
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in fpeat allocation. We stop. we need kjpindex words = ',kjpindex
       STOP 'sechiba_init'
    END IF
    fpeat(:) = undef_sechiba

    ALLOCATE (mc_peat_above(kjpindex,nvm),stat=ier)
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in mc_peat_above allocation. We stop. we need kjpindex words = ',kjpindex,nvm
       STOP 'sechiba_init'
    END IF

    ALLOCATE (shumdiag_peat(kjpindex, nslm,nvm),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'sechiba_init','Pb in alloc for shumdiag_peat','','')

    ALLOCATE (wtpmax(kjpindex,nvm),stat=ier)
    IF (ier.NE.0) THEN
       WRITE (numout,*) ' error in wtpmax allocation. We stop. we need kjpindex words = ',kjpindex,nvm
       STOP 'sechiba_init'
    END IF

!    ALLOCATE(veget_max_adjusted(kjpindex, nvm), STAT=ier)
!    IF (ier /= 0) CALL ipslerr_p(3,'slowproc_init','Problem in allocation of variable veget_max_adjusted','','')
...
!!!qcj++ peatland
    ...
    IF ( ALLOCATED (fwet_new)) DEALLOCATE (fwet_new)
    IF ( ALLOCATED (liqwt_ratio)) DEALLOCATE (liqwt_ratio)
    IF ( ALLOCATED (fpeat)) DEALLOCATE (fpeat)
    IF ( ALLOCATED (mc_peat_above)) DEALLOCATE (mc_peat_above)
    IF ( ALLOCATED (shumdiag_peat)) DEALLOCATE (shumdiag_peat)
    !IF (ALLOCATED (veget_max_adjusted)) DEALLOCATE (veget_max_adjusted)
    IF ( ALLOCATED (wtpmax)) DEALLOCATE (wtpmax)
    IF ( ALLOCATED (wtp_year)) DEALLOCATE (wtp_year)
